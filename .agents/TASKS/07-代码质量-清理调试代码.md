# 任务：清理调试代码

**优先级**: 🔴 P0 - 严重
**类型**: 代码质量
**预估工时**: 1-2 小时
**影响范围**: 1 个文件（主要），其他文件少量

---

## 问题描述

`agent.repository.ts` 存在大量调试日志和调试函数，影响代码可读性和性能。这些调试代码应该在开发完成后移除。

---

## 违规位置

**文件**: `apps/api/src/repositories/agent.repository.ts`

### 1. 调试日志（9 处）

```typescript
// 第 118-122 行
console.log('[AgentRepo] findByIdAndOrg - id:', id, 'orgId:', orgId)
console.log('[AgentRepo] findByIdAndOrg result length:', result.length)

// 第 135-139 行
console.log('[AgentRepo] debugFindByOrgId - orgId:', orgId)
console.log('[AgentRepo] debugFindByOrgId result length:', result.length)

// 第 151 行
console.log('[AgentRepo] findByOrg called with orgId:', orgId)

// 第 197 行
console.log('[AgentRepo] Executing basic query for orgId:', orgId)

// 第 201 行
console.log('[AgentRepo] Basic query result length:', result.length)

// 第 208 行
console.log('[AgentRepo] Count result:', countResult)

// 第 214 行
console.log('[AgentRepo] Returning result with', result.length, 'agents')
```

### 2. 调试函数

```typescript
// 第 134-141 行 - 应该删除
export async function debugFindByOrgId(orgId: string): Promise<AgentRow[]> {
  console.log('[AgentRepo] debugFindByOrgId - orgId:', orgId)
  const result = await sql`SELECT * FROM agents WHERE org_id = ${orgId}`
  console.log('[AgentRepo] debugFindByOrgId result length:', result.length)
  return result as unknown as AgentRow[]
}
```

---

## 修复方案

### 1. 删除所有调试日志

```typescript
// ❌ 删除这些行
console.log('[AgentRepo] findByIdAndOrg - id:', id, 'orgId:', orgId)
console.log('[AgentRepo] findByIdAndOrg result length:', result.length)
// ... 其他 console.log
```

### 2. 删除调试函数

```typescript
// ❌ 删除整个函数
export async function debugFindByOrgId(orgId: string): Promise<AgentRow[]> {
  // ...
}
```

### 3. 如果需要保留日志，使用 logger

```typescript
// ✅ 使用项目 logger，设置为 DEBUG 级别
import { createLogger } from '../lib/logger'

const logger = createLogger('agent-repository')

export async function findByIdAndOrg(id: string, orgId: string): Promise<AgentRow | null> {
  logger.debug('查询 Agent', { id, orgId })

  const result = await sql`
    SELECT * FROM agents
    WHERE id = ${id} AND org_id = ${orgId}
  `

  logger.debug('查询结果', { found: result.length > 0 })

  return result[0] || null
}
```

---

## 完整修复代码

```typescript
// apps/api/src/repositories/agent.repository.ts

import { sql } from '../lib/db'
import { createLogger } from '../lib/logger'

const logger = createLogger('agent-repository')

export interface AgentRow {
  id: string
  org_id: string
  name: string
  description: string | null
  system_prompt: string
  // ... 其他字段
}

/**
 * 根据 ID 和组织 ID 查询 Agent
 */
export async function findByIdAndOrg(id: string, orgId: string): Promise<AgentRow | null> {
  // ✅ 移除调试日志，或使用 logger.debug
  const result = await sql`
    SELECT * FROM agents
    WHERE id = ${id} AND org_id = ${orgId} AND deleted_at IS NULL
  `

  return result[0] as AgentRow || null
}

/**
 * 根据组织 ID 查询 Agent 列表
 */
export async function findByOrg(params: ListAgentParams): Promise<PaginatedResult<AgentRow>> {
  const { orgId, page = 1, limit = 20, search, isActive } = params

  // ✅ 移除调试日志
  const offset = (page - 1) * limit
  const actualLimit = Math.min(limit, MAX_PAGE_SIZE)

  // 构建查询
  let result: AgentRow[]
  let countResult: { count: number }[]

  if (search && isActive !== undefined) {
    result = await sql`
      SELECT * FROM agents
      WHERE org_id = ${orgId}
      AND name ILIKE ${`%${search}%`}
      AND is_active = ${isActive}
      AND deleted_at IS NULL
      ORDER BY created_at DESC
      LIMIT ${actualLimit} OFFSET ${offset}
    `
    countResult = await sql`
      SELECT COUNT(*) as count FROM agents
      WHERE org_id = ${orgId}
      AND name ILIKE ${`%${search}%`}
      AND is_active = ${isActive}
      AND deleted_at IS NULL
    `
  } else if (search) {
    // ... 其他情况
  }

  // ✅ 移除调试日志
  return {
    data: result,
    meta: {
      total: Number(countResult[0].count),
      page,
      limit: actualLimit,
      totalPages: Math.ceil(Number(countResult[0].count) / actualLimit)
    }
  }
}

// ❌ 删除 debugFindByOrgId 函数
```

---

## 修复清单

### 删除调试日志
- [ ] 第 118 行: `console.log('[AgentRepo] findByIdAndOrg - id:...')`
- [ ] 第 122 行: `console.log('[AgentRepo] findByIdAndOrg result length:...')`
- [ ] 第 135 行: `console.log('[AgentRepo] debugFindByOrgId - orgId:...')`
- [ ] 第 139 行: `console.log('[AgentRepo] debugFindByOrgId result length:...')`
- [ ] 第 151 行: `console.log('[AgentRepo] findByOrg called with orgId:...')`
- [ ] 第 197 行: `console.log('[AgentRepo] Executing basic query...')`
- [ ] 第 201 行: `console.log('[AgentRepo] Basic query result length:...')`
- [ ] 第 208 行: `console.log('[AgentRepo] Count result:...')`
- [ ] 第 214 行: `console.log('[AgentRepo] Returning result with...')`

### 删除调试函数
- [ ] 删除 `debugFindByOrgId` 函数（第 134-141 行）

### 检查其他文件
- [ ] 搜索其他文件中的调试代码
- [ ] 确保没有其他 `debug*` 函数

---

## 测试验证

### 1. 代码检查
```bash
# 检查是否还有调试日志
grep -rn "console.log.*AgentRepo" apps/api/src

# 检查是否还有 debug 函数
grep -rn "debugFind" apps/api/src
```

### 2. 运行测试
```bash
cd apps/api
pnpm test
```

### 3. 功能验证
- [ ] Agent 列表查询正常
- [ ] Agent 详情查询正常
- [ ] Agent 创建/更新/删除正常

---

## 注意事项

1. **不要删除有用的日志**
   - 错误日志应保留
   - 边界条件日志应保留（用 logger.warn）
   - 只删除纯调试用途的日志

2. **使用正确的日志级别**
   - DEBUG: 开发调试信息（生产环境不输出）
   - INFO: 正常操作信息
   - WARN: 边界条件、潜在问题
   - ERROR: 错误和异常

3. **确保功能不受影响**
   - 删除调试代码前确认功能正常
   - 删除后运行完整测试

---

## 完成标准

- [ ] 所有调试 console.log 已删除
- [ ] debugFindByOrgId 函数已删除
- [ ] 没有其他调试代码残留
- [ ] 所有测试通过
- [ ] 功能验证通过
- [ ] 代码审查通过

---

## 相关文档

- [编码规范 - 日志规范](.claude/rules/coding-standards.md#日志规范)
- [任务 01 - 统一日志使用](01-编码规范-统一日志使用.md)

# 任务：修复竞态条件

**优先级**: 🔴 P0 - 严重
**类型**: 并发规范
**预估工时**: 0.5-1 小时
**影响范围**: 1 个文件

---

## 问题描述

`auth.service.ts` 中的密码重置请求限流存在竞态条件。`exists` 和 `setWithExpiry` 是两个独立操作，在高并发场景下，多个请求可能同时通过检查。

---

## 违规位置

**文件**: `apps/api/src/services/auth.service.ts:368-372`

```typescript
// ❌ 错误 - 存在竞态条件
const hasRecentRequest = await redis.exists(requestThrottleKey)
if (hasRecentRequest) {
    return
}
await redis.setWithExpiry(requestThrottleKey, '1', PASSWORD_RESET_REQUEST_TTL_SECONDS)
```

**问题分析**:
1. 请求 A 执行 `exists` → 返回 false
2. 请求 B 执行 `exists` → 返回 false（A 还未 set）
3. 请求 A 执行 `setWithExpiry`
4. 请求 B 执行 `setWithExpiry`
5. 结果：两个请求都通过了限流

---

## 修复方案

### 方案 1: 使用 Redis SET NX EX（推荐）

```typescript
// ✅ 正确 - 原子操作
const result = await redis.set(
  requestThrottleKey,
  '1',
  'EX',
  PASSWORD_RESET_REQUEST_TTL_SECONDS,
  'NX'  // Only set if Not eXists
)

if (!result) {
  // Key 已存在，说明最近有请求
  return
}

// Key 设置成功，继续处理
```

**优点**:
- 原子操作，无竞态条件
- Redis 原生支持
- 性能最优

### 方案 2: 使用 Lua 脚本

```typescript
// ✅ 正确 - Lua 脚本保证原子性
const THROTTLE_SCRIPT = `
  local key = KEYS[1]
  local ttl = tonumber(ARGV[1])

  if redis.call('EXISTS', key) == 1 then
    return 0  -- Key exists, throttled
  end

  redis.call('SETEX', key, ttl, '1')
  return 1  -- Success
`

const result = await redis.eval(
  THROTTLE_SCRIPT,
  1,
  requestThrottleKey,
  PASSWORD_RESET_REQUEST_TTL_SECONDS
)

if (result === 0) {
  return  // Throttled
}
```

**优点**:
- 原子操作
- 更灵活，可以添加复杂逻辑

**缺点**:
- 需要维护 Lua 脚本
- 稍微复杂

---

## 完整修复代码

```typescript
// apps/api/src/services/auth.service.ts

export async function requestPasswordReset(email: string): Promise<void> {
  // 1. 查找用户
  const user = await userRepository.findByEmail(email)
  if (!user) {
    // 不暴露用户是否存在
    return
  }

  // 2. 检查请求限流（修复竞态条件）
  const requestThrottleKey = `password_reset_request:${user.id}`

  // ✅ 使用 SET NX EX 原子操作
  const canProceed = await redis.set(
    requestThrottleKey,
    '1',
    'EX',
    PASSWORD_RESET_REQUEST_TTL_SECONDS,
    'NX'
  )

  if (!canProceed) {
    logger.warn('密码重置请求过于频繁', {
      userId: user.id,
      email: user.email
    })
    return
  }

  // 3. 生成重置 Token
  const resetToken = generateSecureToken()
  const resetTokenHash = await hashToken(resetToken)

  // 4. 保存到 Redis
  const resetKey = `password_reset:${resetToken}`
  await redis.setWithExpiry(
    resetKey,
    JSON.stringify({
      userId: user.id,
      email: user.email,
      createdAt: Date.now()
    }),
    PASSWORD_RESET_TOKEN_TTL_SECONDS
  )

  // 5. 发送邮件
  await emailService.sendPasswordResetEmail(user.email, resetToken)

  logger.info('密码重置邮件已发送', {
    userId: user.id,
    email: user.email
  })
}
```

---

## Redis 客户端适配

检查 `apps/api/src/lib/redis.ts` 是否支持 SET NX EX：

```typescript
// 如果使用 ioredis
export async function setNX(
  key: string,
  value: string,
  ttlSeconds: number
): Promise<boolean> {
  const result = await redis.set(key, value, 'EX', ttlSeconds, 'NX')
  return result === 'OK'
}

// 或者直接使用
const result = await redis.set(key, value, 'EX', ttl, 'NX')
```

---

## 测试验证

### 1. 单元测试
```typescript
describe('requestPasswordReset 竞态条件测试', () => {
  it('并发请求应该只有一个成功', async () => {
    const email = 'test@example.com'

    // 并发发送 10 个请求
    const promises = Array(10).fill(null).map(() =>
      authService.requestPasswordReset(email)
    )

    await Promise.all(promises)

    // 验证只发送了一封邮件
    expect(emailService.sendPasswordResetEmail).toHaveBeenCalledTimes(1)
  })

  it('超过 TTL 后应该可以再次请求', async () => {
    const email = 'test@example.com'

    // 第一次请求
    await authService.requestPasswordReset(email)
    expect(emailService.sendPasswordResetEmail).toHaveBeenCalledTimes(1)

    // 立即再次请求，应该被限流
    await authService.requestPasswordReset(email)
    expect(emailService.sendPasswordResetEmail).toHaveBeenCalledTimes(1)

    // 等待 TTL 过期
    await new Promise(resolve => setTimeout(resolve, PASSWORD_RESET_REQUEST_TTL_SECONDS * 1000 + 100))

    // 再次请求，应该成功
    await authService.requestPasswordReset(email)
    expect(emailService.sendPasswordResetEmail).toHaveBeenCalledTimes(2)
  })
})
```

### 2. 压力测试
```typescript
describe('高并发压力测试', () => {
  it('1000 个并发请求应该只有一个成功', async () => {
    const email = 'test@example.com'

    const promises = Array(1000).fill(null).map(() =>
      authService.requestPasswordReset(email)
    )

    await Promise.all(promises)

    expect(emailService.sendPasswordResetEmail).toHaveBeenCalledTimes(1)
  })
})
```

---

## 修复清单

- [ ] 检查 Redis 客户端是否支持 SET NX EX
- [ ] 修改 `requestPasswordReset` 方法
- [ ] 添加单元测试
- [ ] 添加并发测试
- [ ] 运行测试验证
- [ ] 代码审查

---

## 注意事项

1. **Redis 版本要求**
   - SET NX EX 需要 Redis 2.6.12+
   - 项目应该满足此要求

2. **错误处理**
   - Redis 连接失败时的降级策略
   - 考虑使用内存回退（但会失去分布式限流）

3. **日志记录**
   - 记录限流触发事件
   - 便于监控和调试

---

## 完成标准

- [ ] 竞态条件已修复
- [ ] 使用 Redis 原子操作
- [ ] 单元测试通过
- [ ] 并发测试通过
- [ ] 代码审查通过

---

## 相关文档

- [并发规范 - Redis 原子操作](.claude/rules/concurrency.md#redis-原子操作)
- [Redis SET 命令文档](https://redis.io/commands/set/)

...
### Phase 1: 添加常量定义
- [ ] 在 `apps/api/src/constants/config.ts` 中添加所有常量
- [ ] 添加注释说明每个常量的用途

### Phase 2: Services 层修复
- [ ] `chat.service.ts` - 历史消息、标题长度
- [ ] `skill.service.ts` - Skill 相关限制
- [ ] `api-keys.service.ts` - API Key 前缀
- [ ] `skill-retry-rollback.service.ts` - 重试配置

### Phase 3: Lib 层修复
- [ ] `redis.ts` - Redis 重试配置

### Phase 4: Adapters 层修复
- [ ] `runtime.adapter.ts` - SSE 协议前缀

### Phase 5: Routes 层修复
- [ ] `sessions.ts` - Zod 验证长度
- [ ] `users.ts` - Zod 验证长度
- [ ] `mcp.ts` - Zod 验证长度
- [ ] 其他路由文件

### Phase 6: Web 层修复
- [ ] `apps/web/hooks/useSSE.ts` - SSE 协议前缀
- [ ] `apps/web/lib/utils.ts` - 字符串截断

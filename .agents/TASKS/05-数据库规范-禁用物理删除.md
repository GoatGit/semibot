# 任务：禁用物理删除

**优先级**: 🔴 P0 - 严重
**类型**: 数据库规范
**预估工时**: 1-2 小时
**影响范围**: 3 个 Repository

---

## 问题描述

部分 Repository 仍使用 `DELETE FROM` 物理删除数据，违反了数据库规范。物理删除会导致：
1. 数据无法恢复
2. 违反审计要求
3. 可能破坏数据完整性

---

## 规范要求

根据 `.claude/rules/database.md`:

**核心表使用软删除机制，禁止物理删除。**

```typescript
// ❌ 错误 - 物理删除
await sql`DELETE FROM table_name WHERE id = ${id}`

// ✅ 正确 - 软删除
await sql`
  UPDATE table_name
  SET deleted_at = NOW(), deleted_by = ${userId}, updated_at = NOW()
  WHERE id = ${id}
`
```

---

## 违规位置

### 1. Memory Repository ❌
**文件**: `apps/api/src/repositories/memory.repository.ts:206`

```typescript
// ❌ 错误 - 物理删除
export async function remove(id: string): Promise<boolean> {
  const result = await sql`
    DELETE FROM memories WHERE id = ${id} RETURNING id
  `
  return result.length > 0
}

// ✅ 修复 - 软删除
export async function softDelete(id: string, deletedBy: string): Promise<boolean> {
  const result = await sql`
    UPDATE memories
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE id = ${id}
    RETURNING id
  `
  return result.length > 0
}
```

### 2. Session Repository ❌
**文件**: `apps/api/src/repositories/session.repository.ts:207`

```typescript
// ❌ 错误 - 物理删除
export async function remove(id: string, orgId: string): Promise<boolean> {
  const result = await sql`
    DELETE FROM sessions
    WHERE id = ${id} AND org_id = ${orgId}
  `
  return result.length > 0
}

// ✅ 修复 - 软删除
export async function softDelete(id: string, orgId: string, deletedBy: string): Promise<boolean> {
  const result = await sql`
    UPDATE sessions
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE id = ${id} AND org_id = ${orgId}
    RETURNING id
  `
  return result.length > 0
}
```

### 3. Message Repository ❌
**文件**: `apps/api/src/repositories/message.repository.ts:160`

```typescript
// ❌ 错误 - 物理删除
export async function deleteBySessionId(sessionId: string): Promise<number> {
  const result = await sql`
    DELETE FROM messages
    WHERE session_id = ${sessionId}
  `
  return result.length
}

// ✅ 修复 - 软删除
export async function softDeleteBySessionId(sessionId: string, deletedBy: string): Promise<number> {
  const result = await sql`
    UPDATE messages
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE session_id = ${sessionId}
    RETURNING id
  `
  return result.length
}
```

---

## 修复步骤

### Phase 1: 检查表是否有 deleted_at 字段

```sql
-- 检查 memories 表
SELECT column_name
FROM information_schema.columns
WHERE table_name = 'memories'
AND column_name IN ('deleted_at', 'deleted_by');

-- 检查 sessions 表
SELECT column_name
FROM information_schema.columns
WHERE table_name = 'sessions'
AND column_name IN ('deleted_at', 'deleted_by');

-- 检查 messages 表
SELECT column_name
FROM information_schema.columns
WHERE table_name = 'messages'
AND column_name IN ('deleted_at', 'deleted_by');
```

**如果字段不存在，需要先创建迁移脚本添加字段。**

### Phase 2: Repository 层修复

#### 2.1 Memory Repository
```typescript
// 1. 重命名旧方法（保留兼容）
/**
 * @deprecated 使用 softDelete 代替
 */
export async function remove(id: string): Promise<boolean> {
  console.warn('[MemoryRepository] remove() 已废弃，请使用 softDelete()')
  return softDelete(id, 'system')
}

// 2. 添加新方法
export async function softDelete(id: string, deletedBy: string): Promise<boolean> {
  const result = await sql`
    UPDATE memories
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE id = ${id}
    RETURNING id
  `
  return result.length > 0
}
```

#### 2.2 Session Repository
```typescript
export async function softDelete(id: string, orgId: string, deletedBy: string): Promise<boolean> {
  const result = await sql`
    UPDATE sessions
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE id = ${id} AND org_id = ${orgId}
    RETURNING id
  `
  return result.length > 0
}
```

#### 2.3 Message Repository
```typescript
// 批量软删除
export async function softDeleteBySessionId(sessionId: string, deletedBy: string): Promise<number> {
  const result = await sql`
    UPDATE messages
    SET deleted_at = NOW(),
        deleted_by = ${deletedBy},
        updated_at = NOW()
    WHERE session_id = ${sessionId}
    RETURNING id
  `
  return result.length
}
```

### Phase 3: Service 层修复

#### 3.1 更新 Service 调用
```typescript
// ❌ 错误
await memoryRepository.remove(memoryId)

// ✅ 正确
await memoryRepository.softDelete(memoryId, userId)
```

#### 3.2 级联软删除
```typescript
// 删除 Session 时，同时软删除关联的 Messages
export async function deleteSession(sessionId: string, orgId: string, userId: string): Promise<void> {
  // 1. 软删除 Session
  await sessionRepository.softDelete(sessionId, orgId, userId)

  // 2. 软删除关联的 Messages
  await messageRepository.softDeleteBySessionId(sessionId, userId)
}
```

---

## 修复清单

### Repository 层
- [ ] `memory.repository.ts`
  - [ ] 添加 `softDelete` 方法
  - [ ] 废弃 `remove` 方法
- [ ] `session.repository.ts`
  - [ ] 添加 `softDelete` 方法
  - [ ] 废弃 `remove` 方法
- [ ] `message.repository.ts`
  - [ ] 添加 `softDeleteBySessionId` 方法
  - [ ] 废弃 `deleteBySessionId` 方法

### Service 层
- [ ] 更新所有调用 `remove` 的地方
- [ ] 传入 `deletedBy` 参数（userId）
- [ ] 实现级联软删除逻辑

### 数据库迁移（如需要）
- [ ] 为 `memories` 表添加 `deleted_at`, `deleted_by`
- [ ] 为 `sessions` 表添加 `deleted_at`, `deleted_by`
- [ ] 为 `messages` 表添加 `deleted_at`, `deleted_by`

---

## 测试验证

### 1. 单元测试
```typescript
describe('软删除功能', () => {
  it('softDelete 应该设置 deleted_at 而非物理删除', async () => {
    const memory = await memoryRepository.create(data)

    await memoryRepository.softDelete(memory.id, userId)

    // 直接查询数据库，记录应该存在但有 deleted_at
    const result = await sql`
      SELECT id, deleted_at, deleted_by
      FROM memories
      WHERE id = ${memory.id}
    `
    expect(result[0].deleted_at).not.toBeNull()
    expect(result[0].deleted_by).toBe(userId)
  })

  it('级联软删除应该删除关联记录', async () => {
    const session = await sessionRepository.create(data)
    const message = await messageRepository.create({ sessionId: session.id, ... })

    await sessionService.deleteSession(session.id, orgId, userId)

    // Session 和 Message 都应该被软删除
    const sessionResult = await sessionRepository.findById(session.id)
    const messageResult = await messageRepository.findById(message.id)

    expect(sessionResult).toBeNull()
    expect(messageResult).toBeNull()
  })
})
```

### 2. 数据恢复测试
```typescript
describe('数据恢复', () => {
  it('软删除的数据可以恢复', async () => {
    const memory = await memoryRepository.create(data)
    await memoryRepository.softDelete(memory.id, userId)

    // 恢复数据
    await sql`
      UPDATE memories
      SET deleted_at = NULL, deleted_by = NULL
      WHERE id = ${memory.id}
    `

    // 应该可以再次查询到
    const result = await memoryRepository.findById(memory.id)
    expect(result).not.toBeNull()
  })
})
```

---

## 注意事项

1. **数据迁移**
   - 如果表没有 `deleted_at` 字段，需要先运行迁移
   - 迁移脚本应该添加字段但不影响现有数据

2. **级联删除**
   - 删除父记录时，考虑是否需要级联删除子记录
   - 例如：删除 Session 时，是否删除关联的 Messages

3. **性能考虑**
   - 软删除会增加表大小
   - 考虑定期归档或清理旧的软删除记录
   - 为 `deleted_at` 添加索引

4. **审计要求**
   - 始终记录 `deleted_by`
   - 保留删除时间 `deleted_at`

---

## 完成标准

- [ ] 所有物理删除已替换为软删除
- [ ] 所有软删除操作记录 `deleted_by`
- [ ] 级联删除逻辑已实现
- [ ] 所有测试通过
- [ ] 代码审查通过

---

## 相关文档

- [数据库规范 - 软删除](.claude/rules/database.md#软删除)
- [迁移脚本](database/migrations/007_add_soft_delete_and_audit.sql)

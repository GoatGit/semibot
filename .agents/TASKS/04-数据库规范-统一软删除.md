# 任务：统一软删除实现

**优先级**: 🔴 P0 - 严重
**类型**: 数据库规范
**预估工时**: 2-3 小时
**影响范围**: 6 个 Repository

---

## 问题描述

虽然数据库迁移脚本已为核心表添加了 `deleted_at` 和 `deleted_by` 字段，但 Repository 层的查询未全面过滤已删除记录，导致已删除数据仍可被查询到。

---

## 规范要求

根据 `.claude/rules/database.md`:

```sql
-- 必需字段
deleted_at TIMESTAMP NULL DEFAULT NULL
deleted_by UUID NULL

-- 查询规范
SELECT * FROM users WHERE deleted_at IS NULL

-- 软删除操作
UPDATE users SET deleted_at = NOW(), deleted_by = $1 WHERE id = $2
```

---

## 问题详情

### ✅ 正确实现（参考）
**文件**: `apps/api/src/repositories/skill.repository.ts:123`

```typescript
// ✅ 正确 - 过滤 deleted_at
const result = await sql`
  SELECT * FROM skills
  WHERE id = ${id} AND deleted_at IS NULL
`
```

### ❌ 缺失过滤的 Repository

#### 1. Agent Repository
**文件**: `apps/api/src/repositories/agent.repository.ts:104`

```typescript
// ❌ 错误 - 未过滤 deleted_at
export async function findById(id: string): Promise<AgentRow | null> {
  const result = await sql`SELECT * FROM agents WHERE id = ${id}`
  return result[0] || null
}

// ✅ 修复
export async function findById(id: string): Promise<AgentRow | null> {
  const result = await sql`
    SELECT * FROM agents
    WHERE id = ${id} AND deleted_at IS NULL
  `
  return result[0] || null
}
```

#### 2. Tool Repository
**文件**: `apps/api/src/repositories/tool.repository.ts:102`

```typescript
// ❌ 错误 - 未过滤 deleted_at
export async function findById(id: string): Promise<ToolRow | null> {
  const result = await sql`SELECT * FROM tools WHERE id = ${id}`
  return result[0] || null
}

// ✅ 修复
export async function findById(id: string): Promise<ToolRow | null> {
  const result = await sql`
    SELECT * FROM tools
    WHERE id = ${id} AND deleted_at IS NULL
  `
  return result[0] || null
}
```

#### 3. MCP Repository
**文件**: `apps/api/src/repositories/mcp.repository.ts`

```typescript
// ❌ 错误 - 未过滤 deleted_at
export async function findById(id: string): Promise<McpServerRow | null> {
  const result = await sql`SELECT * FROM mcp_servers WHERE id = ${id}`
  return result[0] || null
}

// ✅ 修复
export async function findById(id: string): Promise<McpServerRow | null> {
  const result = await sql`
    SELECT * FROM mcp_servers
    WHERE id = ${id} AND deleted_at IS NULL
  `
  return result[0] || null
}
```

#### 4. Memory Collection Repository
**文件**: `apps/api/src/repositories/memory.repository.ts`

需要检查所有查询是否过滤 `deleted_at`。

#### 5. API Keys Repository
**文件**: `apps/api/src/repositories/api-keys.repository.ts`

需要检查所有查询是否过滤 `deleted_at`。

---

## 修复步骤

### Phase 1: 检查数据库字段
```sql
-- 确认哪些表有 deleted_at 字段
SELECT table_name, column_name
FROM information_schema.columns
WHERE column_name = 'deleted_at'
AND table_schema = 'public';
```

根据迁移脚本 `007_add_soft_delete_and_audit.sql`，以下表有 `deleted_at`:
- organizations
- users
- agents
- skills
- tools
- mcp_servers
- memory_collections
- memory_documents
- api_keys

### Phase 2: Repository 层修复

#### 2.1 修复所有 findById 方法
```typescript
// 模板
export async function findById(id: string): Promise<Row | null> {
  const result = await sql`
    SELECT * FROM table_name
    WHERE id = ${id} AND deleted_at IS NULL
  `
  return result[0] || null
}
```

#### 2.2 修复所有 findAll/list 方法
```typescript
// 模板
export async function findAll(params: ListParams): Promise<PaginatedResult<Row>> {
  const { orgId, page, limit } = params

  const result = await sql`
    SELECT * FROM table_name
    WHERE org_id = ${orgId}
    AND deleted_at IS NULL  -- ✅ 添加此行
    ORDER BY created_at DESC
    LIMIT ${limit} OFFSET ${(page - 1) * limit}
  `

  return {
    data: result,
    meta: { ... }
  }
}
```

#### 2.3 修复所有 search 方法
```typescript
// 模板
export async function search(params: SearchParams): Promise<Row[]> {
  const { orgId, keyword } = params

  const result = await sql`
    SELECT * FROM table_name
    WHERE org_id = ${orgId}
    AND name ILIKE ${`%${keyword}%`}
    AND deleted_at IS NULL  -- ✅ 添加此行
  `

  return result
}
```

### Phase 3: 统一软删除方法

移除 `skill.repository.ts` 中的动态检查逻辑：

```typescript
// ❌ 移除动态检查
const useDeletedAt = await hasSkillsDeletedAtColumn()
const result = useDeletedAt
  ? await sql`... AND deleted_at IS NULL`
  : await sql`...`

// ✅ 统一实现（假设迁移已完成）
const result = await sql`... AND deleted_at IS NULL`
```

---

## 修复清单

### Repository 层修复
- [x] `agent.repository.ts`
  - [x] findById
  - [x] findByOrg
  - [ ] findAll
  - [ ] search
- [x] `tool.repository.ts`
  - [x] findById
  - [ ] findAll
  - [ ] search
- [x] `mcp.repository.ts`
  - [x] findById
  - [x] findByOrg
  - [ ] findAll
- [ ] `memory.repository.ts`
  - [ ] findById
  - [ ] findAll
  - [ ] search
- [ ] `api-keys.repository.ts`
  - [ ] findById
  - [ ] findByOrg
- [ ] `skill.repository.ts`
  - [ ] 移除动态检查逻辑
  - [ ] 统一使用 deleted_at

### 验证
- [ ] 所有查询都包含 `deleted_at IS NULL`
- [ ] 软删除操作设置 `deleted_at = NOW()`
- [ ] 单元测试通过
- [ ] 集成测试通过

---

## 测试验证

### 1. 单元测试
```typescript
describe('软删除功能', () => {
  it('findById 应该不返回已删除的记录', async () => {
    // 创建记录
    const agent = await agentRepository.create(data)

    // 软删除
    await agentRepository.softDelete(agent.id, orgId)

    // 查询应该返回 null
    const result = await agentRepository.findById(agent.id)
    expect(result).toBeNull()
  })

  it('findAll 应该不包含已删除的记录', async () => {
    // 创建两个记录
    const agent1 = await agentRepository.create(data1)
    const agent2 = await agentRepository.create(data2)

    // 软删除一个
    await agentRepository.softDelete(agent1.id, orgId)

    // 查询应该只返回未删除的
    const result = await agentRepository.findAll({ orgId })
    expect(result.data).toHaveLength(1)
    expect(result.data[0].id).toBe(agent2.id)
  })
})
```

### 2. 数据库验证
```sql
-- 验证软删除记录存在但不可查询
SELECT id, name, deleted_at
FROM agents
WHERE deleted_at IS NOT NULL;

-- 验证应用层查询不返回已删除记录
-- 通过 API 测试
```

---

## 注意事项

1. **向后兼容**
   - 确保所有表都已运行迁移脚本
   - 如果有表未迁移，需要先运行迁移

2. **性能考虑**
   - 为 `deleted_at` 字段添加索引（如果未添加）
   - 考虑定期清理旧的软删除记录

3. **审计要求**
   - 软删除时记录 `deleted_by`
   - 保留删除记录用于审计

---

## 完成标准

- [ ] 所有 Repository 的查询都过滤 `deleted_at IS NULL`
- [ ] 移除动态检查逻辑
- [ ] 所有测试通过
- [ ] 代码审查通过
- [ ] 文档已更新

---

## 相关文档

- [数据库规范 - 软删除](.claude/rules/database.md#软删除)
- [迁移脚本](database/migrations/007_add_soft_delete_and_audit.sql)

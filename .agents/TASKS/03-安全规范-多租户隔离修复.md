# 任务：修复多租户隔离漏洞

**优先级**: 🔴 P0 - 严重
**类型**: 安全规范
**风险等级**: HIGH - 可能导致跨租户数据泄露
**预估工时**: 2-3 小时
**影响范围**: 4 个 Repository，多个 Service

---

## 问题描述

部分 Repository 的 `findById` 方法缺少 `org_id` 验证，可能导致跨租户数据泄露。

---

## 严重问题

### 1. Message Repository ⚠️ HIGH RISK
**文件**: `apps/api/src/repositories/message.repository.ts:90-92`

```typescript
// ❌ 当前实现 - 缺少 org_id 检查
export async function findById(id: string): Promise<MessageRow | null> {
  const result = await sql`SELECT * FROM messages WHERE id = ${id}`
  if (result.length === 0) return null
  return result[0] as unknown as MessageRow
}

// ✅ 建议实现 - 通过 session 关联验证
export async function findByIdAndOrg(id: string, orgId: string): Promise<MessageRow | null> {
  const result = await sql`
    SELECT m.* FROM messages m
    JOIN sessions s ON m.session_id = s.id
    WHERE m.id = ${id} AND s.org_id = ${orgId}
  `
  if (result.length === 0) return null
  return result[0] as unknown as MessageRow
}
```

**风险**: 任何用户可以通过 message_id 访问其他组织的消息

---

### 2. Tool Repository ⚠️ MEDIUM RISK
**文件**: `apps/api/src/repositories/tool.repository.ts:100-109`

```typescript
// ❌ 当前实现 - 缺少 org_id 检查
export async function findById(id: string): Promise<ToolRow | null> {
  const result = await sql`SELECT * FROM tools WHERE id = ${id}`
  if (result.length === 0) return null
  return result[0] as unknown as ToolRow
}

// ✅ 建议实现 - 添加 org_id 参数
export async function findByIdAndOrg(id: string, orgId: string): Promise<ToolRow | null> {
  const result = await sql`
    SELECT * FROM tools
    WHERE id = ${id}
    AND (org_id = ${orgId} OR is_builtin = true)
  `
  if (result.length === 0) return null
  return result[0] as unknown as ToolRow
}
```

**说明**: Tool 支持 `is_builtin` 标志，内置工具可以跨组织访问

---

### 3. Skill Repository ⚠️ MEDIUM RISK
**文件**: `apps/api/src/repositories/skill.repository.ts:120-130`

```typescript
// ❌ 当前实现 - 缺少 org_id 检查
export async function findById(id: string): Promise<SkillRow | null> {
  const useDeletedAt = await hasSkillsDeletedAtColumn()
  const result = useDeletedAt
    ? await sql`SELECT * FROM skills WHERE id = ${id} AND deleted_at IS NULL`
    : await sql`SELECT * FROM skills WHERE id = ${id}`
  return result[0] || null
}

// ✅ 建议实现 - 添加 org_id 参数
export async function findByIdAndOrg(id: string, orgId: string): Promise<SkillRow | null> {
  const result = await sql`
    SELECT * FROM skills
    WHERE id = ${id}
    AND (org_id = ${orgId} OR is_builtin = true)
    AND deleted_at IS NULL
  `
  return result[0] || null
}
```

---

### 4. Agent Repository ⚠️ MEDIUM RISK
**文件**: `apps/api/src/repositories/agent.repository.ts:102-111`

```typescript
// ❌ 当前实现 - 缺少 org_id 检查
export async function findById(id: string): Promise<AgentRow | null> {
  const result = await sql`SELECT * FROM agents WHERE id = ${id}`
  if (result.length === 0) return null
  return result[0] as unknown as AgentRow
}

// ✅ 已有 findByIdAndOrg 方法，但 findById 仍被使用
```

**说明**:
- 已有 `findByIdAndOrg` 方法（L120）
- Service 层在公开 Agent 访问时有验证（L215）
- 但 `findById` 方法仍存在，可能被误用

---

## 修复步骤

### Phase 1: Repository 层修复

#### 1.1 Message Repository
```typescript
// 1. 添加新方法
export async function findByIdAndOrg(id: string, orgId: string): Promise<MessageRow | null> {
  const result = await sql`
    SELECT m.* FROM messages m
    JOIN sessions s ON m.session_id = s.id
    WHERE m.id = ${id} AND s.org_id = ${orgId}
  `
  return result[0] || null
}

// 2. 废弃旧方法（添加注释）
/**
 * @deprecated 使用 findByIdAndOrg 代替，以确保多租户隔离
 */
export async function findById(id: string): Promise<MessageRow | null> {
  // 保留以兼容，但应逐步迁移
  const result = await sql`SELECT * FROM messages WHERE id = ${id}`
  return result[0] || null
}
```

#### 1.2 Tool Repository
```typescript
export async function findByIdAndOrg(id: string, orgId: string): Promise<ToolRow | null> {
  const result = await sql`
    SELECT * FROM tools
    WHERE id = ${id}
    AND (org_id = ${orgId} OR is_builtin = true)
  `
  return result[0] || null
}
```

#### 1.3 Skill Repository
```typescript
export async function findByIdAndOrg(id: string, orgId: string): Promise<SkillRow | null> {
  const result = await sql`
    SELECT * FROM skills
    WHERE id = ${id}
    AND (org_id = ${orgId} OR is_builtin = true)
    AND deleted_at IS NULL
  `
  return result[0] || null
}
```

#### 1.4 Agent Repository
```typescript
// 移除 findById 方法，强制使用 findByIdAndOrg
// 或添加 @deprecated 注释
```

---

### Phase 2: Service 层修复

#### 2.1 检查所有调用 findById 的地方
```bash
# 搜索所有 findById 调用
grep -rn "\.findById(" apps/api/src/services --include="*.ts"
```

#### 2.2 替换为 findByIdAndOrg
```typescript
// ❌ 错误
const message = await messageRepository.findById(messageId)

// ✅ 正确
const message = await messageRepository.findByIdAndOrg(messageId, orgId)
if (!message) {
  throw createError(RESOURCE_NOT_FOUND)
}
```

---

### Phase 3: 添加安全警告日志

在所有 Repository 的查询方法中添加：

```typescript
export async function findAll(params: ListParams): Promise<PaginatedResult<Row>> {
  const { orgId, ... } = params

  // ✅ 添加安全警告
  if (!orgId) {
    logger.warn('[Security] 查询未提供 org_id，存在跨租户风险', {
      method: 'findAll',
      repository: 'MessageRepository'
    })
  }

  // ... 查询逻辑
}
```

---

## 修复清单

### Repository 层
- [ ] `message.repository.ts` - 添加 `findByIdAndOrg`
- [ ] `tool.repository.ts` - 添加 `findByIdAndOrg`
- [ ] `skill.repository.ts` - 添加 `findByIdAndOrg`
- [ ] `agent.repository.ts` - 废弃 `findById`

### Service 层
- [ ] 检查所有 `findById` 调用
- [ ] 替换为 `findByIdAndOrg`
- [ ] 添加 `org_id` 验证

### 安全日志
- [ ] 所有 Repository 添加安全警告日志
- [ ] 缺少 `org_id` 时打印 WARN 日志

---

## 测试验证

### 1. 单元测试
```typescript
describe('MessageRepository.findByIdAndOrg', () => {
  it('应该只返回属于指定组织的消息', async () => {
    const message = await messageRepository.findByIdAndOrg(messageId, orgId)
    expect(message).toBeDefined()
    expect(message.session.org_id).toBe(orgId)
  })

  it('应该拒绝访问其他组织的消息', async () => {
    const message = await messageRepository.findByIdAndOrg(messageId, otherOrgId)
    expect(message).toBeNull()
  })
})
```

### 2. 集成测试
```typescript
describe('多租户隔离', () => {
  it('用户A不能访问用户B的消息', async () => {
    // 创建两个组织的数据
    const orgA = await createOrg('Org A')
    const orgB = await createOrg('Org B')

    const messageA = await createMessage(orgA.id)

    // 尝试用 orgB 访问 orgA 的消息
    const result = await messageRepository.findByIdAndOrg(messageA.id, orgB.id)
    expect(result).toBeNull()
  })
})
```

### 3. 手动测试
```bash
# 1. 创建两个组织的测试数据
# 2. 尝试跨组织访问
# 3. 验证返回 404 或 null
```

---

## 完成标准

- [ ] 所有 Repository 都有 `findByIdAndOrg` 方法
- [ ] Service 层不再直接调用 `findById`
- [ ] 所有查询都包含 `org_id` 过滤
- [ ] 添加了安全警告日志
- [ ] 单元测试通过
- [ ] 集成测试通过
- [ ] 代码审查通过

---

## 相关文档

- [安全规范 - 多租户隔离](.claude/rules/security.md#多租户隔离)
- [数据模型设计](docs/design/DATA_MODEL.md)

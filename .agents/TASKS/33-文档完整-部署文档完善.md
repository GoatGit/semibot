# 任务：部署文档完善

**优先级**: 🟢 P2 - 中优先级
**类型**: 文档完整
**预估工时**: 1 天
**影响范围**: docs/

---

## 问题描述

项目缺少完整的部署文档，导致：
1. 部署流程不清晰
2. 环境配置容易出错
3. 运维困难

---

## 需要的文档

| 文档 | 内容 | 状态 |
|------|------|------|
| `docs/deployment/local.md` | 本地开发环境 | ⚠️ 缺失 |
| `docs/deployment/docker.md` | Docker 部署 | ⚠️ 缺失 |
| `docs/deployment/kubernetes.md` | K8s 部署 | ⚠️ 缺失 |
| `docs/deployment/production.md` | 生产环境检查清单 | ⚠️ 缺失 |

---

## 文档内容

### 1. 本地开发环境

```markdown
# 本地开发环境

## 系统要求

| 组件 | 最低版本 | 推荐版本 |
|------|----------|----------|
| Node.js | 18.0.0 | 20.x LTS |
| pnpm | 8.0.0 | 最新 |
| Python | 3.11 | 3.11.x |
| PostgreSQL | 15 | 15.x |
| Redis | 7 | 7.x |

## 安装步骤

### 1. 克隆仓库

```bash
git clone https://github.com/your-org/semibot.git
cd semibot
```

### 2. 安装 Node.js 依赖

```bash
# 安装 pnpm（如果没有）
npm install -g pnpm

# 安装依赖
pnpm install
```

### 3. 安装 Python 依赖

```bash
cd runtime
python -m venv venv
source venv/bin/activate  # Windows: venv\Scripts\activate
pip install -r requirements.txt
pip install -r requirements-dev.txt
cd ..
```

### 4. 配置环境变量

```bash
# 复制环境变量模板
cp .env.example .env

# 编辑配置
# 必须配置以下变量：
# - DATABASE_URL
# - REDIS_URL
# - JWT_SECRET
# - OPENAI_API_KEY 或 ANTHROPIC_API_KEY
```

### 5. 初始化数据库

```bash
# 创建数据库
createdb semibot

# 运行迁移
pnpm db:migrate
```

### 6. 启动开发服务器

```bash
# 启动所有服务
pnpm dev

# 或分别启动
pnpm --filter @semibot/api dev
pnpm --filter @semibot/web dev
cd runtime && python -m src.main
```

## 开发端口

| 服务 | 端口 | 说明 |
|------|------|------|
| Web | 3000 | 前端应用 |
| API | 3001 | 后端 API |
| Runtime | 8000 | Python Runtime |
| PostgreSQL | 5432 | 数据库 |
| Redis | 6379 | 缓存 |

## 常见问题

### 数据库连接失败

检查 PostgreSQL 服务是否启动：
```bash
pg_isready
```

### Redis 连接失败

检查 Redis 服务是否启动：
```bash
redis-cli ping
```
```

### 2. Docker 部署

```markdown
# Docker 部署

## 前提条件

- Docker >= 20.10
- Docker Compose >= 2.0

## 快速启动

```bash
# 构建并启动所有服务
docker-compose up -d

# 查看日志
docker-compose logs -f

# 停止服务
docker-compose down
```

## docker-compose.yml

```yaml
version: '3.8'

services:
  web:
    build:
      context: .
      dockerfile: apps/web/Dockerfile
    ports:
      - "3000:3000"
    environment:
      - NEXT_PUBLIC_API_URL=http://api:3001
    depends_on:
      - api

  api:
    build:
      context: .
      dockerfile: apps/api/Dockerfile
    ports:
      - "3001:3001"
    environment:
      - DATABASE_URL=postgresql://semibot:password@db:5432/semibot
      - REDIS_URL=redis://redis:6379
      - JWT_SECRET=${JWT_SECRET}
    depends_on:
      - db
      - redis

  runtime:
    build:
      context: ./runtime
      dockerfile: Dockerfile
    ports:
      - "8000:8000"
    environment:
      - DATABASE_URL=postgresql://semibot:password@db:5432/semibot
      - REDIS_URL=redis://redis:6379
      - OPENAI_API_KEY=${OPENAI_API_KEY}
    depends_on:
      - db
      - redis

  db:
    image: pgvector/pgvector:pg15
    ports:
      - "5432:5432"
    environment:
      - POSTGRES_USER=semibot
      - POSTGRES_PASSWORD=password
      - POSTGRES_DB=semibot
    volumes:
      - postgres_data:/var/lib/postgresql/data

  redis:
    image: redis:7-alpine
    ports:
      - "6379:6379"
    volumes:
      - redis_data:/data

volumes:
  postgres_data:
  redis_data:
```

## Dockerfile 示例

### API Dockerfile

```dockerfile
# apps/api/Dockerfile
FROM node:20-alpine AS builder

WORKDIR /app
COPY package.json pnpm-lock.yaml ./
COPY packages ./packages
COPY apps/api ./apps/api

RUN npm install -g pnpm
RUN pnpm install --frozen-lockfile
RUN pnpm --filter @semibot/api build

FROM node:20-alpine AS runner

WORKDIR /app
COPY --from=builder /app/apps/api/dist ./dist
COPY --from=builder /app/node_modules ./node_modules

EXPOSE 3001
CMD ["node", "dist/index.js"]
```

### Runtime Dockerfile

```dockerfile
# runtime/Dockerfile
FROM python:3.11-slim

WORKDIR /app

COPY requirements.txt .
RUN pip install --no-cache-dir -r requirements.txt

COPY src ./src

EXPOSE 8000
CMD ["python", "-m", "src.main"]
```

## 环境变量

创建 `.env` 文件：

```bash
# 数据库
DATABASE_URL=postgresql://semibot:password@db:5432/semibot

# Redis
REDIS_URL=redis://redis:6379

# 认证
JWT_SECRET=your-secret-key

# LLM
OPENAI_API_KEY=sk-xxx
# 或
ANTHROPIC_API_KEY=sk-ant-xxx
```

## 健康检查

```bash
# 检查 API
curl http://localhost:3001/health

# 检查 Runtime
curl http://localhost:8000/health
```
```

### 3. 生产环境检查清单

```markdown
# 生产环境检查清单

## 部署前检查

### 安全

- [ ] 所有密钥使用环境变量
- [ ] JWT_SECRET 足够复杂（32+ 字符）
- [ ] 数据库密码足够复杂
- [ ] 禁用调试模式
- [ ] 配置 HTTPS
- [ ] 配置 CORS 白名单
- [ ] 配置 CSP 头

### 数据库

- [ ] 运行所有迁移
- [ ] 配置连接池大小
- [ ] 启用 SSL 连接
- [ ] 配置定期备份
- [ ] 设置只读副本（如需）

### 缓存

- [ ] Redis 启用持久化
- [ ] 配置 Redis 密码
- [ ] 配置 maxmemory 策略

### 日志

- [ ] 配置日志级别为 info
- [ ] 配置日志轮转
- [ ] 配置日志聚合（如 ELK）

### 监控

- [ ] 配置健康检查端点
- [ ] 配置 APM（如 New Relic、Datadog）
- [ ] 配置告警规则
- [ ] 配置 Uptime 监控

### 性能

- [ ] 启用 gzip 压缩
- [ ] 配置 CDN
- [ ] 配置 HTTP 缓存头
- [ ] 优化数据库索引

## 环境变量

| 变量 | 必需 | 说明 |
|------|------|------|
| `NODE_ENV` | ✅ | 设为 `production` |
| `DATABASE_URL` | ✅ | PostgreSQL 连接字符串 |
| `REDIS_URL` | ✅ | Redis 连接字符串 |
| `JWT_SECRET` | ✅ | JWT 签名密钥 |
| `OPENAI_API_KEY` | ⚠️ | OpenAI API Key（二选一） |
| `ANTHROPIC_API_KEY` | ⚠️ | Anthropic API Key（二选一） |
| `LOG_LEVEL` | ❌ | 日志级别，默认 `info` |
| `CORS_ORIGINS` | ❌ | CORS 白名单 |

## 扩容建议

### 水平扩容

- API 服务可水平扩容，使用负载均衡
- Runtime 服务可水平扩容，使用负载均衡
- 数据库使用只读副本分担读压力

### 垂直扩容

| 服务 | 最低配置 | 推荐配置 |
|------|----------|----------|
| API | 1 CPU, 1GB RAM | 2 CPU, 2GB RAM |
| Runtime | 2 CPU, 2GB RAM | 4 CPU, 4GB RAM |
| PostgreSQL | 2 CPU, 4GB RAM | 4 CPU, 8GB RAM |
| Redis | 1 CPU, 1GB RAM | 2 CPU, 2GB RAM |

## 回滚计划

1. 保留最近 3 个版本的镜像
2. 数据库迁移支持回滚
3. 配置回滚脚本
4. 记录回滚步骤

## 故障排查

### API 无响应

1. 检查容器状态：`docker ps`
2. 检查日志：`docker logs api`
3. 检查数据库连接
4. 检查 Redis 连接

### 数据库连接失败

1. 检查 DATABASE_URL 配置
2. 检查网络连通性
3. 检查连接数限制
4. 检查 SSL 配置

### 内存不足

1. 检查内存使用：`docker stats`
2. 增加容器内存限制
3. 优化查询，减少内存占用
```

---

## 修复清单

### 文档创建
- [ ] 创建 `docs/deployment/` 目录
- [ ] 创建 `docs/deployment/local.md`
- [ ] 创建 `docs/deployment/docker.md`
- [ ] 创建 `docs/deployment/kubernetes.md`
- [ ] 创建 `docs/deployment/production.md`

### 配置文件
- [ ] 完善 `docker-compose.yml`
- [ ] 创建各服务 Dockerfile
- [ ] 创建 `.env.example`

---

## 完成标准

- [ ] 部署文档完整
- [ ] 配置文件可用
- [ ] 部署步骤可执行
- [ ] 代码审查通过

---

## 相关文档

- [架构设计](docs/design/ARCHITECTURE.md)
- [本地开发](docs/development/local.md)

...
class LocalShortTermMemory:
    ...
    def _session_file(self, session_id: str) -> Path:
        return self.base_dir / f"{validate_session_id(session_id)}.md"
    ...
    async def replace(self, session_id: str, content: str) -> None:
        file_path = self._session_file(session_id)
        file_path.parent.mkdir(parents=True, exist_ok=True)
...
class LocalMemoryIndex:
    ...
    def save(self, session_id: str, data: dict[str, Any]) -> None:
        path = self.base_dir / f"{validate_session_id(session_id)}.json"

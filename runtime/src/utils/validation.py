...
def validate_session_id(value: str | None, field_name: str = "session_id") -> str:
    if not isinstance(value, str):
        raise InvalidInputError(f"{field_name} must be a string")
    normalized = value.strip()
    if not _SESSION_ID_PATTERN.fullmatch(normalized):
        raise InvalidInputError(f"Invalid {field_name}: {value!r}")
    return normalized


def validate_file_id(value: str | None, field_name: str = "file_id") -> str:
    if not isinstance(value, str):
        raise InvalidInputError(f"{field_name} must be a string")
    normalized = value.strip().lower()
    if not _FILE_ID_PATTERN.fullmatch(normalized):
        raise InvalidInputError(f"Invalid {field_name}: {value!r}")
    return normalized


def ensure_relative_path(path: str | None, field_name: str = "path") -> Path:
    ...
    if candidate.is_absolute():
        raise InvalidInputError(f"Absolute paths are not allowed: {path!r}")
    if any(part == ".." for part in candidate.parts):
        raise InvalidInputError(f"Path traversal is not allowed: {path!r}")
    return candidate

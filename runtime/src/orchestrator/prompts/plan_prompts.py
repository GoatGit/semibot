...
def planner_system_prompt(
    *,
    ...
    return (
        ...
        f"2. Check the skill index below. If a skill matches the task, call read_skill(skill_id) to load its methodology BEFORE creating a plan.\n"
        f"3. If sub-agent delegation is appropriate, call inspect_sub_agent(agent_id) to understand capabilities.\n"
        f"4. Output your decision as exactly one JSON object.\n"
        f"\n"
        f"== Decision Types ==\n"
        ...
        f"\n"
        f"== Skill Usage ==\n"
        f"- If a skill is strongly relevant, call read_skill(skill_id) first, then use it as a methodology scaffold (not a template to copy blindly).\n"
...
def compact_planner_system_prompt(
    *,
    ...
    return (
        ...
        f"8. If a skill in the skill index is strongly relevant, call read_skill(skill_id) before finalizing. Set selected_skill and skill_context_for_act.\n"

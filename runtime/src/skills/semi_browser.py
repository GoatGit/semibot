...
class _BrowserSession:
    ...
    created_at: float
    last_used_at: float
    element_refs: dict[str, dict]


class SemiBrowserTool(BaseTool):
    """Automate browser interactions using highly optimized aria snapshots."""

    def __init__(self) -> None:
        self.default_timeout_ms = int(os.getenv("SEMIBOT_BROWSER_TIMEOUT_MS") or "30000")
        ...
        self.max_text_length = int(os.getenv("SEMIBOT_BROWSER_MAX_TEXT_LENGTH") or "20000")
    ...
    def description(self) -> str:
        return (
            "Automate interactive browser actions for JS-rendered or stateful pages. "
            "Use this when page interaction, DOM state, login, or screenshots are required beyond simple HTTP fetch."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "selector": {"type": "string", "description": "CSS selector for click/type/extract/wait_for (legacy fallback). Prefer ref when a snapshot provides one."},
                "ref": {"type": "string", "description": "Element reference ID (e.g. 'e1', 'e2') from the latest snapshot. Preferred over selector for click/type."},
                ...
                "wait_until": {
                    ...
                    "description": "Navigation readiness target for action=open.",
                ...
                "path": {
                    "type": "string",
                    "description": "Output path for screenshot. Relative paths are stored under the configured browser artifact directory and returned as result.path.",
                },
                "max_chars": {"type": "integer", "description": "Maximum characters returned by extract_text/get_html."},
            },
            "required": [],
        }
    ...
    async def _build_aria_snapshot(self, session: _BrowserSession) -> tuple[str, bool]:
        js_code = r"""() => {
            let counter = 1;
            const elements = [];
            ...
            function traverse(node, depth) {
                ...
                if (['script', 'style', 'noscript', 'svg', 'canvas', 'video', 'audio', 'img'].includes(tag)) return;

                const isInteractive = ['a', 'button', 'input', 'select', 'textarea'].includes(tag) || node.hasAttribute('onclick') || node.getAttribute('role') === 'button' || node.getAttribute('role') === 'link';
                const role = node.getAttribute('role') || (tag === 'a' ? 'link' : tag);
                
                if (isInteractive) {
                    const ref = "e" + counter++;
                    node.setAttribute("data-semi-ref", ref);
                    
                    let name = node.innerText || node.getAttribute("aria-label") || node.getAttribute("alt") || node.value || node.placeholder || "";
                    if (name) name = name.trim().replace(/\n/g, ' ').substring(0, 100);
                    
                    elements.push(`${'  '.repeat(depth)}- ${role} "${name}" [ref=${ref}]`);
                    return;
                }
                ...
                if (['h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'p', 'span', 'div', 'li'].includes(tag) && directText) {
                    let name = node.innerText;
                    if (name) name = name.trim().replace(/\n/g, ' ').substring(0, 200);
                    if (name) {
                        elements.push(`${'  '.repeat(depth)}- ${tag} "${name}"`);
                    }
                    return;
                }
        ...
        snapshot = await session.page.evaluate(js_code)
        
        full_text = str(snapshot or "")
        truncated = False
        ...
        try:
            ...
            if action_name == "click":
                ...
                if not selector and not ref:
                    return ToolResult.error_result("ref or selector is required for action=click")
                    
                if ref:
                    try:
                        locator = page.locator(f'[data-semi-ref="{ref}"]')
                        await locator.click(timeout=timeout_ms)
                        clicked_target = f"ref={ref}"
                    except Exception as e:
                        return ToolResult.error_result(f"Interaction failed on {ref}. The element might be out of date. Extract snapshot again: {e}")
                else:
                    await page.click(selector, timeout=timeout_ms)
                    clicked_target = selector
                    
                return ToolResult.success_result({"session_id": session_id, "url": page.url, "clicked": clicked_target})
            ...
            if action_name == "type":
                ...
                if not selector and not ref:
                    return ToolResult.error_result("ref or selector is required for action=type")
                    
                if ref:
                    try:
                        locator = page.locator(f'[data-semi-ref="{ref}"]')
                        await locator.fill(text, timeout=timeout_ms)
                        typed_target = f"ref={ref}"
                    except Exception as e:
                        return ToolResult.error_result(f"Interaction failed on {ref}. The element might be out of date. Extract snapshot again: {e}")
                else:
                    await page.fill(selector, text, timeout=timeout_ms)
                    typed_target = selector
                    
                return ToolResult.success_result({"session_id": session_id, "typed": typed_target, "length": len(text)})
            ...
            if action_name == "extract_text":
                # We now completely replace the raw innerText extraction with our Aria Snapshot builder
                snapshot_text, truncated = await self._build_aria_snapshot(session)

...
class CsvXlsxTool(BaseTool):
    ...
    def __init__(self) -> None:
        ...
        self.max_return_rows = int(os.getenv("SEMIBOT_CSV_XLSX_MAX_RETURN_ROWS") or "500")
    ...
    def description(self) -> str:
        return (
            "Read, write, filter, aggregate, or pivot CSV/XLSX datasets under the configured root. "
            "Returns preview rows plus total_rows/truncated, and optionally generated output files."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "path": {"type": "string", "description": "Input file path under the configured root."},
                "output_path": {"type": "string", "description": "Output file path under the configured root for write/export operations."},
                ...
                "data": {"description": "Rows used for write/transform operations, typically a list of objects."},
                "filters": {
                    "type": "array",
                    "description": "Filter rules: [{field, op, value}] where op in eq/ne/gt/gte/lt/lte/contains/in. For op=in, value may be a list or comma-separated string.",
                },
                "group_by": {"type": "array", "description": "Fields used for aggregate grouping."},
                "metrics": {
                    "type": "array",
                    "description": "Aggregate metrics [{field, op, as}] where op in count/sum/avg/min/max. count may use field='*'.",
                },
                "pivot": {
                    "type": "object",
                    "description": "Pivot config: {index, columns, values, agg} where agg is one of sum/avg/min/max/count.",
                },
                "limit": {"type": "integer", "description": "Maximum preview rows returned in tool output. It does not limit actual processing."},
            },
            "required": ["action"],
        }
    ...
    def _persist_generated(self, path: Path) -> list[dict[str, Any]]:
        ...
        suffix = path.suffix.lower()
        if suffix == ".xlsx":
            meta["artifact_role"] = "table_xlsx"
            meta["artifact_format"] = "xlsx"
        elif suffix == ".csv":
            meta["artifact_role"] = "table_csv"
            meta["artifact_format"] = "csv"
        else:
            meta["artifact_role"] = "file"
            meta["artifact_format"] = suffix.lstrip(".") or "binary"
        meta["artifact_medium"] = "file"
        meta["artifact_type"] = "tabular_output"
        meta["artifact_name"] = "tabular output"
        meta["artifact_purpose"] = "spreadsheet or table deliverable"
        meta["is_likely_final"] = True
        return [meta]

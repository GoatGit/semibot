...
class JsonTransformTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Transform JSON data via a small selector subset, field mappings, or render templates. "
            "Modes are selector (expression), mapping, or template. In auto mode, template wins first, then mapping, then selector."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "expression": {
                    "type": "string",
                    "description": "Selector expression using the supported JSONPath/JMESPath-like subset. Used in selector mode.",
                },
                "language": {
                    "type": "string",
                ...
                "mapping": {
                    "type": "object",
                    "description": "Field mapping: outputKey -> selector expression. Used in mapping mode.",
                },
                "template": {
                    "type": "string",
                    "description": "Render template with placeholders, e.g. {{$.user.name}}. Used in template mode.",
                },
                "default_value": {"description": "Fallback value when selector misses."},
                "keep_nulls": {"type": "boolean", "default": False, "description": "Only applies to mapping mode. Keep keys whose selector resolves to null/missing."},
            },
            "required": ["data"],
        }

...
class HttpClientTool(BaseTool):
    ...
    def __init__(self) -> None:
        self.timeout_ms = int(os.getenv("SEMIBOT_HTTP_CLIENT_TIMEOUT_MS") or "15000")
        self.retry_attempts = int(os.getenv("SEMIBOT_HTTP_CLIENT_RETRY_ATTEMPTS") or "2")
        self.max_response_chars = int(os.getenv("SEMIBOT_HTTP_CLIENT_MAX_RESPONSE_CHARS") or "20000")
    ...
    def description(self) -> str:
        return (
            "Generic REST HTTP client for APIs and webhooks, with auth, retries, and response truncation. "
            "Use this for structured HTTP endpoints rather than webpage content extraction. "
            "Returns {request, response}, where response includes status_code, headers, body, optional parsed json, and truncated."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "path": {
                    "type": "string",
                    "description": "Path appended to configured apiEndpoint when url is omitted. If both url and path are provided, url wins.",
                },
                "headers": {"type": "object", "description": "HTTP headers as a string map."},
                "query": {"type": "object", "description": "Query parameters appended to the final request URL."},
                "body": {"description": "Raw request body for non-GET methods. Use for plain text, bytes, or form-like payloads."},
                "json_body": {"description": "JSON request body. Preferred for JSON APIs and takes precedence over body."},
                "auth_type": {
                    "type": "string",
                    "enum": ["none", "bearer", "basic", "api_key"],
                    "default": "none",
                    "description": "Authentication mode. none disables per-call auth injection.",
                },
                "auth_token": {"type": "string", "description": "Token value for bearer or api_key auth."},
                ...
                "retry_attempts": {"type": "integer", "description": "Retry attempts on transport errors and 5xx responses, using short incremental backoff."},
                "max_response_chars": {
                    "type": "integer",
                    "description": "Maximum number of response body characters returned. Longer bodies are truncated and marked with response.truncated=true.",
                },
            },
            "required": [],
        }

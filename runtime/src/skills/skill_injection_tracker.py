...
class SkillInjectionTracker:
    ...
    max_cached_resources: int = _MAX_CACHED_RESOURCES
    ...
    def mark_resource_read(
        ...
        *,
        ...
        key = (skill_id, file_path)
        # Evict oldest entry if at capacity and this is a new key
        if key not in self._resource_read and len(self._resource_read) >= self.max_cached_resources:
            oldest_key = next(iter(self._resource_read))
            logger.warning(
                "Resource cache at capacity, evicting oldest entry "
                "(current: %d, limit: %d, evicted: %s)",
                len(self._resource_read),
                self.max_cached_resources,
                oldest_key,
            )
            del self._resource_read[oldest_key]
        self._resource_read[key] = ResourceReadRecord(
            skill_id=skill_id,
            file_path=file_path,
            content=content,
            content_mtime=content_mtime,

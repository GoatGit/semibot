...
class SqlQueryReadonlyTool(BaseTool):
    ...
    def __init__(self) -> None:
        self.default_timeout_ms = int(os.getenv("SEMIBOT_SQL_READ_TIMEOUT_MS") or "15000")
        self.default_max_rows = int(os.getenv("SEMIBOT_SQL_READ_MAX_ROWS") or "200")
    ...
    def description(self) -> str:
        return (
            "Run read-only SQL queries against configured SQLite or PostgreSQL data sources. "
            "Only single SELECT/CTE statements are allowed. Returns {database, columns, rows, row_count, truncated}."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            ...
            "properties": {
                ...
                "database": {
                    "type": "string",
                    "description": "Target database alias configured in the tool config allowlist.",
                },
                "params": {
                    "description": "Optional positional SQL params list for placeholders supported by the configured driver.",
                },

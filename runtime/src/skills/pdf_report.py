...
class PdfReportTool(BaseTool):
    ...
    def description(self) -> str:
        return (
            "Generate a structured PDF report file with title, summary, sections, tables, charts, and conclusion. "
            "Returns generated_files metadata for the created PDF."
        )
    ...
    def parameters(self) -> dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "filename": {"type": "string", "default": "report.pdf", "description": "Output PDF filename. Relative names are written under the tool working directory and returned through generated_files."},
                ...
                "sections": {
                    ...
                    "description": (
                        ...
                        "When sections are provided, they take precedence over context_data auto-building."
                    ),
                },
                "conclusion": {"type": "string", "description": "Conclusion paragraph."},
                "context_data": {"description": "Optional JSON string/object used to auto-build sections when sections is empty. Prefer an object with results:[...]."},
            },
            "required": [],
        }
    ...
    @staticmethod
    def _create_work_dir(runtime_context: Any | None) -> Path:
        session_root = PdfReportTool._resolve_session_working_root(runtime_context)
        if session_root is None:
            return Path(tempfile.mkdtemp(prefix="semibot-pdf-report-"))
        work_dir = session_root / "tool_runs" / f"pdf_report_{uuid.uuid4().hex[:12]}"
    ...
    async def execute(
        ...
        **kwargs: Any,
        ...
        try:
            work_dir = self._create_work_dir(kwargs.get("_runtime_context"))
            out_path = work_dir / safe_filename
            pdf = FPDF()
            pdf.add_page()
            ...
            generated_files = []
            if meta:
                meta["source_path"] = str(out_path.resolve())
                meta["artifact_role"] = "report_pdf"
                meta["artifact_medium"] = "file"
                meta["artifact_format"] = "pdf"
                meta["artifact_type"] = "research_report"
                meta["artifact_name"] = "report pdf"
                meta["artifact_purpose"] = "derived PDF report deliverable"

...
from src.product.services import RuntimeServiceOptions, StackOptions
...
def _resolve_cli_version() -> str:
    try:
        release_loader = ProductConfigLoader()
        active_release = release_loader.active_release_payload()
        active_version = str(active_release.get("active_version") or "").strip()
        if active_version:
            return active_version
    except Exception:
        pass
    try:
        package_file = Path(__file__).resolve().parents[2] / "package.json"
        return str(json.loads(package_file.read_text(encoding="utf-8")).get("version") or "dev")
    except Exception:
        return "dev"
...
def _augment_payload(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if VERBOSE_ENABLED:
        ...
        meta.setdefault("output", OUTPUT_FORMAT)
...
def _bootstrap_from_args(args: argparse.Namespace) -> None:
    if not bool(getattr(args, "bootstrap_runtime_home", True)):
        return
...
def _apply_interactive_init_prompts(args: argparse.Namespace) -> None:
    if bool(getattr(args, "no_input", False)):
        return
    should_prompt = bool(getattr(args, "interactive", False))
    if not should_prompt and not (
        sys.stdin.isatty()
        and sys.stdout.isatty()
        and not getattr(args, "openai_api_key", None)
        and not getattr(args, "anthropic_api_key", None)
        and not getattr(args, "default_model", None)
        and not getattr(args, "update_manifest_url", None)
    ):
        return
    if not should_prompt:
        should_prompt = _prompt_yes_no("Run interactive init prompts for model and API keys?", default=False)
    if not should_prompt:
        return
    ...
    current = loader.load(
        {
            ...
            "update_manifest_url": getattr(args, "update_manifest_url", None),
        }
    )

    print(_clr("Semibot init", "1;38;5;208"))
    args.runtime_host = _prompt_text("Runtime host", current.runtime.host)
    args.runtime_port = _prompt_int("Runtime port", current.runtime.port)
    args.api_port = _prompt_int("API port", current.ports.api)
    args.web_port = _prompt_int("Web port", current.ports.web)
    args.default_model = _prompt_text("Default model", current.llm.default_model or "") or None
    openai_default = current.llm.openai_api_key or ""
    anthropic_default = current.llm.anthropic_api_key or ""
    update_manifest_default = current.updates.manifest_url or ""
    args.openai_api_key = _prompt_text("OpenAI API key", openai_default) or None
    args.anthropic_api_key = _prompt_text("Anthropic API key", anthropic_default) or None
    args.update_manifest_url = _prompt_text("Update manifest URL", update_manifest_default) or None
...
def cmd_init(args: argparse.Namespace) -> int:
    _apply_interactive_init_prompts(args)
    try:
        payload = build_product_init_payload(
            db_path=args.db_path,
            rules_path=args.rules_path,
            runtime_host=str(getattr(args, "runtime_host", "127.0.0.1")),
            runtime_port=int(getattr(args, "runtime_port", 8765)),
...
def cmd_upgrade(args: argparse.Namespace) -> int:
    try:
        ...
        _print_json(_error_payload(resource="upgrade", action="install", code="UPGRADE_FAILED", message=str(exc)))
        return EXIT_EXTERNAL_ERROR
    _print_json({"version": CLI_VERSION, "mode": "upgrade", **payload})
    return EXIT_SUCCESS


def cmd_doctor(args: argparse.Namespace) -> int:
    payload = build_product_doctor_payload(
        db_path=args.db_path,
        rules_path=args.rules_path,
    ...
    return EXIT_SUCCESS if payload.get("ok") else EXIT_CONFIG_ERROR
...
def _gateway_instance_health_report(instance: dict[str, Any]) -> dict[str, Any]:
    ...
    bot_bindings, partial_bindings, duplicate_binding_ids = _normalize_chat_bindings_for_health(
        config_map.get("botBindings")
    )
    checks["botBindingCount"] = len(bot_bindings)
    if partial_bindings > 0:
        errors.append(f"bot_bindings_partial:{partial_bindings}")
    if duplicate_binding_ids:
        warnings.append(f"bot_bindings_duplicate:{len(duplicate_binding_ids)}")
...
def _serve_action_error(action: str, message: str, *, details: dict[str, Any] | None = None) -> int:
...
def cmd_serve_start(args: argparse.Namespace) -> int:
    try:
        _print_json(
            {
                ...
                **build_runtime_service_payload(
                    RuntimeServiceOptions(
                        ...
                        cron_jobs_json=args.cron_jobs_json,
                    ),
                    action="start",
                ),
            }
        ...
        return _serve_action_error("start", str(exc))
...
def cmd_serve_stop(args: argparse.Namespace) -> int:
    try:
        ...
        return _serve_action_error("stop", str(exc))
...
def cmd_serve_restart(args: argparse.Namespace) -> int:
    try:
        ...
        return _serve_action_error("restart", str(exc))
    ...
    _print_json(
            {
                ...
                **build_product_status_payload(
                    StackOptions(
                        ...
                        runtime_name=getattr(args, "runtime_name", None),
                        runtime_port=getattr(args, "runtime_port", None),
                        api_port=getattr(args, "api_port", None),
                        web_port=getattr(args, "web_port", None),
        ...
        _print_json(
            {
                ...
                **build_product_logs_payload(
                    ...
                    lines=max(0, int(getattr(args, "lines", 100))),
                ),
            }
        )
        return EXIT_SUCCESS
    except RuntimeError as exc:
        _print_json(_error_payload(resource="logs", action="tail", code="LOGS_FAILED", message=str(exc)))
        return EXIT_EXTERNAL_ERROR


def cmd_product_up(args: argparse.Namespace) -> int:
    try:
        _print_json(
            {
                "version": CLI_VERSION,
                ...
                **build_product_up_payload(
                    StackOptions(
                        ...
                        api_port=getattr(args, "api_port", None),
                        web_port=getattr(args, "web_port", None),
                        runtime_db_path=getattr(args, "runtime_db_path", None),
                        runtime_rules_path=getattr(args, "runtime_rules_path", None),
                        runtime_heartbeat_interval=getattr(args, "runtime_heartbeat_interval", None),
                        runtime_cron_jobs_json=getattr(args, "runtime_cron_jobs_json", None),
                        with_runtime=bool(getattr(args, "with_runtime", True)),
                        wait_for_health=bool(getattr(args, "wait_for_health", True)),
                        health_timeout_seconds=float(getattr(args, "health_timeout", 30.0)),
                    )
                ),
            }
        )
        ...
        _print_json(_error_payload(resource="up", action="start", code="STACK_START_FAILED", message=str(exc)))
        return EXIT_EXTERNAL_ERROR


def cmd_product_down(args: argparse.Namespace) -> int:
    try:
        _print_json(
            {
                "version": CLI_VERSION,
        ...
        _print_json(_error_payload(resource="down", action="stop", code="STACK_STOP_FAILED", message=str(exc)))
        return EXIT_EXTERNAL_ERROR


def cmd_ui(args: argparse.Namespace) -> int:
    action = str(getattr(args, "ui_action", "open") or "open")
    options = StackOptions(
        ...
        runtime_cron_jobs_json=getattr(args, "runtime_cron_jobs_json", None),
        with_runtime=bool(getattr(args, "with_runtime", True)),
        wait_for_health=not bool(getattr(args, "no_wait", False)),
        health_timeout_seconds=float(getattr(args, "health_timeout", 30.0)),
    )
    if action == "open":
        try:
            ...
            _print_json(_error_payload(resource="ui", action="open", code="UI_OPEN_FAILED", message=str(exc)))
            return EXIT_EXTERNAL_ERROR
    if action in {"start", "stop", "restart"}:
        try:
            payload = build_product_ui_payload(
                options,
                action=action,
                should_open=not bool(getattr(args, "no_open", False)),
            )
            _print_json({"version": CLI_VERSION, "mode": f"ui-{action}", **payload})
            return EXIT_SUCCESS
        except RuntimeError as exc:
            _print_json(
                _error_payload(
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--verbose", action="store_true", help="Include CLI metadata in output payloads")
    ...
    register_product_command_parsers(
        ...
        defaults=ProductCommandDefaults(
            ...
            runtime_url=str(os.getenv("SEMIBOT_RUNTIME_URL", "http://127.0.0.1:8765")),
        ),
    ...
    serve_parser = subparsers.add_parser("serve", help="Manage runtime service via built-in supervisor")
    ...
    serve_start_parser.add_argument("--name", default="semibot-runtime", help="Runtime service name")
    serve_start_parser.add_argument("--host", default=None, help="Bind host (default: config or built-in)")
    serve_start_parser.add_argument("--port", type=int, default=None, help="Bind port (default: config or built-in)")
    serve_start_parser.add_argument("--db-path", default=None, help="SQLite DB path (default: config or built-in)")
    serve_start_parser.add_argument("--rules-path", default=None, help="Rules path (default: config or built-in)")
    ...
    serve_stop_parser.add_argument("--name", default="semibot-runtime", help="Runtime service name")
    serve_stop_parser.add_argument("--port", type=int, default=None, help="Runtime bind port to cleanup")
    ...
    serve_restart_parser.add_argument("--name", default="semibot-runtime", help="Runtime service name")
    serve_restart_parser.add_argument("--host", default=None, help="Bind host (default: config or built-in)")
    serve_restart_parser.add_argument("--port", type=int, default=None, help="Bind port (default: config or built-in)")
    serve_restart_parser.add_argument("--db-path", default=None, help="SQLite DB path (default: config or built-in)")
    serve_restart_parser.add_argument("--rules-path", default=None, help="Rules path (default: config or built-in)")
    ...
    ui_common_parser = argparse.ArgumentParser(add_help=False)
    ui_common_parser.add_argument(
        "--name-prefix",
        default=None,
        help="Service name prefix (default: project-isolated)",
    )
    ui_common_parser.add_argument("--api-port", type=int, default=None, help="API port (default: config or built-in)")
    ui_common_parser.add_argument("--web-port", type=int, default=None, help="Web port (default: config or built-in)")
    ui_common_parser.add_argument(
        "--runtime-name",
        default=None,
        help="Runtime service name (default: semibot-runtime)",
    )
    ui_common_parser.add_argument("--runtime-host", default=None, help="Runtime bind host (default: config or built-in)")
    ui_common_parser.add_argument("--runtime-port", type=int, default=None, help="Runtime bind port (default: config or built-in)")
    ui_common_parser.add_argument("--runtime-db-path", default=None, help="Runtime SQLite DB path (default: config or built-in)")
    ui_common_parser.add_argument("--runtime-rules-path", default=None, help="Runtime rules path (default: config or built-in)")
    ui_common_parser.add_argument(
    ...
    ui_common_parser.add_argument(
        ...
        action="store_false",
        help="Return immediately after spawning services",
    )
    ui_common_parser.add_argument(
        "--health-timeout",
        type=float,
    ...
    ui_common_parser.add_argument(
        "--no-open",
        action="store_true",
        help="Do not auto-open browser when using bare `semibot ui`",
    )

    ui_parser = subparsers.add_parser("ui", help="Open or manage UI/API stack", parents=[ui_common_parser])
    ui_parser.set_defaults(with_runtime=True)
    ui_parser.set_defaults(wait_for_health=True)
    ui_parser.set_defaults(func=cmd_ui)
    ui_subparsers = ui_parser.add_subparsers(dest="ui_action", required=False)

    ui_start_parser = ui_subparsers.add_parser(
        "start",
        help="Start runtime/UI/API services in background",
        parents=[ui_common_parser],
    )
    ui_start_parser.set_defaults(wait_for_health=True)
    ui_start_parser.set_defaults(func=cmd_ui)

    ui_stop_parser = ui_subparsers.add_parser("stop", help="Stop runtime/UI/API services", parents=[ui_common_parser])
    ui_stop_parser.set_defaults(wait_for_health=True)
    ui_stop_parser.set_defaults(func=cmd_ui)

    ui_restart_parser = ui_subparsers.add_parser(
        "restart",
        help="Restart runtime/UI/API services",
        parents=[ui_common_parser],
    )
    ui_restart_parser.set_defaults(wait_for_health=True)
...
def main() -> None:
    global COLOR_ENABLED, OUTPUT_FORMAT, TRACE_ID, VERBOSE_ENABLED
    parser = build_parser()
    args = parser.parse_args()
    OUTPUT_FORMAT = "json" if getattr(args, "json", False) else getattr(args, "output", "table")
    VERBOSE_ENABLED = bool(getattr(args, "verbose", False))

...
6. For PDF generation with Chinese content, you MUST use the CJK font: pdf.add_font('HiraginoGB', '', '/System/Library/Fonts/Hiragino Sans GB.ttc') (do NOT pass uni=True — it is deprecated). Then use pdf.set_font('HiraginoGB', size=...). Do NOT use style='B'/'I' with custom fonts unless a separate bold/italic font file is registered. Use font size for visual hierarchy. Use new_x/new_y instead of deprecated ln parameter: pdf.cell(200, 10, text='Title', new_x='LMARGIN', new_y='NEXT', align='C'). PYTHON CODE RULE: NEVER use f-strings with nested quotes like f'{{item['key']}}' — this causes SyntaxError. Use concatenation or temp variables instead: title = item['title']; pdf.cell(0, 10, text=title). PDF MARGIN RULE: Always set safe margins at the start: pdf.set_margins(10, 10, 10) and pdf.set_auto_page_break(auto=True, margin=15). For multi_cell, ALWAYS use w=0 (auto full width) and ensure font size <= 12 for body text. NEVER call multi_cell right after cell without new_x='LMARGIN' — the x position may leave no room. PDF URL RULE: When rendering URLs in PDF, NEVER use multi_cell for URLs — long URLs cause "Not enough horizontal space" errors. Instead truncate or use cell with link: url = item['url'][:80]; pdf.cell(0, 8, text=url, link=item['url'], new_x='LMARGIN', new_y='NEXT').
7. DEPENDENCIES & ORDERING: Think about data flow between steps. A step that consumes output from earlier steps MUST come after them and MUST have "parallel": false. CRITICAL: If the plan has a search step followed by code_executor steps that use the search results, ALL steps MUST have "parallel": false. Only truly independent steps with zero data dependencies may use "parallel": true.
8. DATA PASSING BETWEEN STEPS: When a code_executor step needs data from a previous step (e.g. search results for report generation), you MUST pass that data via the "context_data" parameter (a JSON string). The data will be written to "context.json" in the working directory. The code should read it with: import json; data = json.load(open('context.json', encoding='utf-8')). DO NOT embed large data as string literals in the code — always use context_data. Structure context_data as: {{"results": [{{"title": "...", "url": "...", "content": "..."}},...]}}.
...
13. CAPABILITY GAP HANDLING: If the request cannot be completed with current tools, and "skill_installer" is available, create a plan to:
   (a) search skill hub (e.g. https://clawhub.ai/skills?sort=downloads) via web_fetch/browser tools,
   (b) get a downloadable skill package URL,
   (c) call skill_installer to install it,
   (d) continue execution using the newly installed tool.
   Do NOT fabricate completion without attempting this fallback.
...
"""
        ...
        example_code = (
            ...
            "pdf.cell(0, 10, text='AI趋势报告', new_x='LMARGIN', new_y='NEXT', align='C')\n"

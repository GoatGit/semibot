...
class AuditLogger:
    ...
    @staticmethod
    def _context_org_id(context: RuntimeSessionContext) -> str:
        metadata = context.metadata if isinstance(context.metadata, dict) else {}
        org_id = metadata.get("org_id")
        if not isinstance(org_id, str) or not org_id.strip():
            raise ValueError("RuntimeSessionContext.metadata.org_id is required for audit logging")
        return org_id.strip()
    ...
    async def log_action_started(
        ...
        event = AuditEvent(
            ...
            org_id=self._context_org_id(context),

...
class SemiGraphAdapter(RuntimeAdapter):
    ...
    def _create_llm_provider(self) -> OpenAIProvider | None:
        ...
        extra_custom_keys: list[tuple[str, str]] = []
        if isinstance(api_keys, dict):
            for key, value in api_keys.items():
                key_name = str(key or "")
                key_value = str(value or "")
                if key_name.startswith("custom:") and key_value.strip():
                    extra_custom_keys.append((key_name, key_value))

        openai_key = api_keys.get("openai") or os.getenv("OPENAI_API_KEY")
        custom_key = api_keys.get("custom") or os.getenv("CUSTOM_LLM_API_KEY")
        api_key = openai_key or custom_key or (extra_custom_keys[0][1] if extra_custom_keys else None)
        if not api_key:
            return None
        ...
        if openai_key:
            ...
            if not base_url and extra_custom_keys and isinstance(providers_cfg, dict):
                custom_provider_cfg = providers_cfg.get(extra_custom_keys[0][0])

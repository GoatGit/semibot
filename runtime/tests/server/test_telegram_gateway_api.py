...
from src.gateway.channels.telegram import ingress as telegram_ingress
...
async def test_telegram_webhook_ingestion_and_text_approval(tmp_path: Path):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        assert str(sent[-1]["payload"]["text"]).startswith("done:")
        assert "hello semibot" in str(sent[-1]["payload"]["text"])
...
async def test_telegram_text_approval_triggers_resume_followup(tmp_path: Path):
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        ...
        assert any(text.startswith("done:") and "hello semibot" in text for text in sent_texts)
...
async def test_telegram_webhook_file_attachment_is_downloaded_and_passed_to_task(
    ...
    monkeypatch.setattr(telegram_ingress, "download_attachments", _fake_download)
...
@pytest.mark.asyncio
async def test_telegram_dynamic_chat_discovery_accepts_unknown_chat_ids(tmp_path: Path):
    db_path = tmp_path / "events.db"
    rules_path = tmp_path / "rules.json"
    _write_rules(rules_path)
    app = create_app(db_path=str(db_path), rules_path=str(rules_path))
...
@pytest.mark.asyncio
async def test_telegram_bot_bindings_route_to_bound_agents(tmp_path: Path):
    db_path = tmp_path / "events.db"
    rules_path = tmp_path / "rules.json"
    _write_rules(rules_path)
    ...
    async with httpx.AsyncClient(transport=transport, base_url="http://testserver") as client:
        created = await client.post(
            ...
            json={
                ...
                "config": {
                    ...
                    "addressingPolicy": {"mode": "all_messages", "executeOnUnaddressed": True},
                    "botBindings": [
                        {"botId": "444444", "agentId": "agent.alpha"}
                    ],
                },
            },
        )
        ...
        assert second.json()["agent_id"] == "agent.alpha"
        ...
        assert fallback.json()["agent_id"] == "agent.alpha"
        ...
        assert observed == ["agent.alpha", "agent.alpha", "agent.alpha"]
        assert len(sent) >= 3

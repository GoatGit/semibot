...
def test_gateway_context_extracts_missing_capability_from_runtime_result():
    runtime_result = {
        "tool_results": [
            {
                "tool_name": "act_terminal",
                "success": False,
                "metadata": {
                    "missing_capability": {
    ...
    extracted = GatewayContextService._extract_missing_capability(runtime_result)

    assert extracted is not None
    assert extracted["intent"] == "authenticated_browser_session"
    assert extracted["preferred_sources"] == ["cli"]


def test_gateway_context_missing_capability_returns_none_when_absent():
    assert GatewayContextService._extract_missing_capability({"tool_results": []}) is None


def test_gateway_context_extracts_proposed_cli_import_from_runtime_result():
    runtime_result = {
        "tool_results": [
            {
                "tool_name": "act_terminal",
    ...
    extracted = GatewayContextService._extract_proposed_cli_import(runtime_result)

    assert extracted is not None
    assert extracted["shape"] == "group"
    assert extracted["command"] == ["opencli", "xiaohongshu"]


@pytest.mark.asyncio
async def test_gateway_context_creates_pending_cli_import_request_from_runtime_result(
    tmp_path: Path,
...
""",
            ...
            "tool_results": [
                {
                    ...
                    "metadata": {
                        "proposed_cli_import": {
                            ...
                            "tool_name": "fake_xhs_import",
                            "reason": "Need xiaohongshu browser automation",
                        }
                    },
                }
            ],
    ...
    async def _on_result(message: str, metadata: dict[str, Any]) -> bool:
        sent.append({"message": message, "metadata": metadata})
        return True

    result = await gateway_context.ingest_message(
        provider="telegram",
        event_payload={"chat_id": "chat_1", "sender_id": "user_1", "chat_type": "dm"},
    ...
    for _ in range(20):
        requests = config_store.list_cli_import_requests(limit=10)
        if requests:
            break
        await asyncio.sleep(0.05)
    assert len(requests) == 1
    assert requests[0]["status"] == "awaiting_approval"
    assert requests[0]["tool_name"] == "fake_xhs_import"
    assert sent
    assert "CLI 导入审批请求" in sent[-1]["message"]
...
async def test_gateway_manager_telegram_ingest_with_addressing_policy(tmp_path: Path):
    ...
    assert len(runs) == 1
    manager.gateway_context.store.update_task_run(
        runs[0]["run_id"],
        status="done",
    ...
    convs = manager.list_gateway_conversations(provider="telegram", limit=10)["data"]
    assert convs[0]["latest_run"]["missing_capability"]["intent"] == "authenticated_browser_session"
    assert convs[0]["latest_run"]["result_metadata"]["missing_capability"]["preferred_sources"] == ["cli"]

    runs = manager.list_gateway_conversation_runs(conv_id, limit=10)["data"]
    assert runs[0]["missing_capability"]["intent"] == "authenticated_browser_session"
    assert runs[0]["result_metadata"]["missing_capability"]["version"] == "1"

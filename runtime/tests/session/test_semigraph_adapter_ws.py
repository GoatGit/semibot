...
async def test_semigraph_adapter_rewrites_premature_final_response(monkeypatch):
    ...
    assert "# current request" in final_response
    assert "PDD Investor Relations" in final_response
...
@pytest.mark.asyncio
async def test_semigraph_adapter_checkpoint_persists_plan_and_execution_state(tmp_path):
    adapter = SemiGraphAdapter(
        client=DummyClient(),
        session_id="sess-checkpoint-contract",
        user_id="user-1",
        init_data={"api_keys": {}, "memory_dir": str(tmp_path)},
        start_payload={"agent_id": "agent-1", "agent_config": {}, "mcp_servers": []},
    )

    plan = ExecutionPlan(
        goal="搜索最新 AI 行业动态并总结",
        ...
        steps=[
            PlanStep(
                id="step-1",
                title="检索并整理最新信息",
                intent="检索最新 AI 行业动态并整理成文本摘要",
            )
        ],
        final_delivery_contract={
            "delivery_goal": "AI 行业动态总结",
    ...
    execution_state = {
        ...
        "observations": ["已检索并整理最新动态"],
        "artifacts_produced": ["AI 行业动态总结"],
    }

    await adapter._save_checkpoint(
        status="completed",
        payload={"message": "搜索最新的 AI 行业动态并总结", "history": []},
        result={
    ...
    cp_dir = tmp_path / "sess-checkpoint-contract" / "checkpoints"
    files = list(cp_dir.glob("*.json"))
    assert files
    payload = files[0].read_text()
    assert '"plan"' in payload
    assert '"execution_state"' in payload
    assert '"final_delivery_contract"' in payload
    assert '"current_date": "2026-03-12"' in payload
    assert '"current_weekday": "Thursday"' in payload
    assert '"current_timezone": "Asia/Shanghai"' in payload

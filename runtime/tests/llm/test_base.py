...
from src.llm.base import LLMConfig, LLMProvider, LLMResponse, _build_response_context
...
class TestMemoryContextNormalization:
    def test_build_response_context_uses_shared_memory_preprocessor(self):
        memory_context = """
<skill_md>internal skill body</skill_md>
...
"""
        ...
        context = _build_response_context(results=None, reflection=None, memory_context=memory_context)

        assert "Conversation memory:" in context
        assert "<skill_md>" not in context
        assert "[system]" not in context
        assert "Latest user turn" in context
        assert "...(assistant history truncated)..." in context

    @pytest.mark.asyncio
    async def test_generate_plan_bad_request_retry_uses_shared_memory_preprocessor(self, sample_llm_config):
        ...
        class TestProvider(LLMProvider):
            ...
            async def chat(self, messages, **kwargs):
                ...
                if len(self.calls) == 1:
                    raise bad_request_cls("context length exceeded")
                return LLMResponse(
                    content='{"goal":"retry","steps":[],"requires_delegation":false,"delegate_to":null}',
                    model=self.model,
                )
...
[user] Keep this user memory

[assistant] """ + ("assistant " * 300) + """

[system] [SYSTEM] REPLAN hidden
...
"""
        ...
        assert result["goal"] == "retry"
        assert len(provider.calls) == 2
        fallback_system_prompt = provider.calls[1][0]["content"]
        assert "Memory context:" in fallback_system_prompt

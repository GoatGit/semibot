...
class TestOpenAIProviderChat:
    ...
    @pytest.mark.asyncio
    async def test_chat_retries_without_optional_params_on_httpstatuserror_400(
        self, provider, sample_messages
    ):
        class HTTP400Error(Exception):
            def __init__(self):
                ...
                self.response = MagicMock(status_code=400, text='{"error":{"message":"unsupported tools"}}')
        ...
        mock_response.usage.total_tokens = 2
        provider.client.chat.completions.create = AsyncMock(
            side_effect=[HTTP400Error(), mock_response]
        )
    ...
    @pytest.mark.asyncio
    async def test_chat_omits_temperature_for_moonshot_provider(self, mock_openai_client, sample_messages):
        with patch("src.llm.openai_provider.AsyncOpenAI"):
            config = LLMConfig(
                model="kimi-k2.5",
                api_key="sk-test",
        ...
        await provider.chat(sample_messages, temperature=0.2)

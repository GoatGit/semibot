...
function startBridge() {
  ...
  const sdkCmd = `node -e "console.error('sdk failed');process.exit(7)"`
...
async function main() {
  ...
  const sessionId = 'sess-sdk-fail-e2e-1'
  ...
  child.stdin.write(
    `${JSON.stringify({
      type: 'cp_response',
      id: memoryReq.id,
      result: { results: [{ content: 'memo-fail' }] },
    })}\n`
  )

  await waitFor(
    collector,
    (lines) => lines.some((x) => x.type === 'execution_error' && x.code === 'SDK_COMMAND_FAILED')
  )

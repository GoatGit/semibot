...
function startBridge() {
  ...
  const sdkCmd = `node -e "setTimeout(()=>{process.stdout.write('late')}, 3000)"`
...
async function main() {
  ...
  const sessionId = 'sess-sdk-timeout-e2e-1'
  ...
  child.stdin.write(
    `${JSON.stringify({
      type: 'cp_response',
      id: memoryReq.id,
      result: { results: [{ content: 'memo-timeout' }] },
    })}\n`
  )

  await waitFor(
    collector,
    (lines) => lines.some((x) => x.type === 'execution_error' && x.code === 'SDK_COMMAND_TIMEOUT')
  )

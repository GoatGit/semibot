...
**Q：上下文策略（原 context-policies）现在走哪个 API？**  
A：统一走 `evolution-capabilities`。`/api/v1/context-policies/*` 仅兼容保留并标记 deprecated，管理面与新功能一律使用 `/api/v1/evolution-capabilities/*`（对象：`hands/reflex/spine/guard/mind`）。
...
| [Tools 配置矩阵](./tool-config-matrix.md) | 内建工具字段、默认值、风险与审批策略一览 |
| [统一控制工具与通道网关](./unified-control-tool-and-channel-gateway.md) | `rule_authoring` 升级为控制面工具、Channel/Gateway 分层、说明注入与防递归设计 |
| [Skills 索引与安装](./skills-index-and-installation.md) | Skills metadata 模型、索引更新、Web 安装（zip/目录）与 runtime 刷新机制 |
...
| [进化机制（重设计）](./evolution-context-system.md) | Capability 进化平台、评测、灰度、发布与回滚 |
| [上下文注入体系（GENE/AGENTS/TOOLS）](./context-injection-system.md) | 策略文档职责、注入时序、版本治理与回滚 |

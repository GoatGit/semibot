 ...
 * 质量退化检查 — 定时任务调用
 */
export async function checkQualityDegradation(): Promise<{
  ...
  const lowSuccessSkills = await EvolvedSkillRepo.findLowSuccessRate(
    DEPRECATION_MAX_SUCCESS_RATE, DEPRECATION_MIN_USE_COUNT
  )
  ...
  // 2. 长期未使用标记候选清理
  const staleSkills = await EvolvedSkillRepo.findStaleSkills(STALE_SKILL_DAYS)

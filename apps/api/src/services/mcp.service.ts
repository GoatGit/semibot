...
export async function createMcpServer(
  ...
  // 系统 MCP 不受组织配额限制
  if (!isSystem) {
    const count = await mcpRepository.countByOrg()
...
export async function getMcpServer(serverId: string): Promise<McpServer> {
  const row = await mcpRepository.findByIdAndOrg(serverId)
...
export async function updateMcpServer(
  ...
  const row = await mcpRepository.update(serverId, {
    name: input.name,
    description: input.description,
    endpoint: input.endpoint,
    transport: input.transport,
...
export async function deleteMcpServer(serverId: string): Promise<void> {
  const deleted = await mcpRepository.softDelete(serverId)
...
export async function testConnection(
  ...
  // 更新状态为连接中
  await mcpRepository.update(serverId, {
    status: 'connecting',
  })
 ...
 */
export async function getOrgMcpServersForRuntime(): Promise<Array<{
  id: string
  name: string
  endpoint: string
  transport: string
  ...
  const servers = await mcpRepository.findActiveByOrg()

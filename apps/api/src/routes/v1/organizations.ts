...
const orgLogger = createLogger('organizations')
...
function handleError(res: Response, error: unknown): void {
  ...
  orgLogger.error('未预期的错误', error as Error)
  res.status(500).json({
    success: false,
...
router.get('/current', authenticate, async (req: AuthRequest, res: Response) => {
  try {
    const orgId = await organizationService.getOrgIdByUserId(req.user!.userId)
    const org = await organizationService.getCurrentOrganization(orgId)

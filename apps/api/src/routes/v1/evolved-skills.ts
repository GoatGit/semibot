...
router.get(
  ...
  combinedRateLimit,
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const skill = await evolvedSkillService.getById(req.params.id)
...
router.post(
  '/:id/review',
  authenticate,
  requirePermission('evolved-skills:write'),
  combinedRateLimit,
...
router.delete(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { userId } = req.user!
    await evolvedSkillService.deprecate(req.params.id, userId)
...
router.post(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    const { userId } = req.user!
    const skill = await evolvedSkillService.promote(req.params.id, userId)
    res.json({ success: true, data: skill })
  })
)

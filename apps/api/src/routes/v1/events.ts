...
router.get(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    if (isUuidSessionId(targetSessionId) && filteredItems.length === 0) {
      try {
        const sessionMessages = await sessionService.getSessionMessages(targetSessionId)
...
router.get(
  ...
  asyncHandler(async (_req: AuthRequest, res: Response) => {
    const dictionary = await eventEngineService.getEventPresentationDictionary()
    res.json({
      success: true,
      data: dictionary,
    })
...
router.put(
  ...
  asyncHandler(async (req: AuthRequest, res: Response) => {
    ...
    const dictionary = await eventEngineService.updateEventPresentationDictionary(payload)
    res.json({
      success: true,
      data: dictionary,
    })

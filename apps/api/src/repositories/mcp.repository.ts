...
export class McpRepositoryImpl {
  async findByIdAndOrg(id: string): Promise<McpServerRow | null> {
    return findByIdAndOrg(id)
  }
}
 ...
 */
export async function findByIdAndOrg(id: string): Promise<McpServerRow | null> {
  return findById(id)
}
 ...
 */
export async function findActiveByOrg(): Promise<McpServerRow[]> {
 ...
 * 更新 MCP Server
 */
export async function update(id: string, data: UpdateMcpServerData): Promise<McpServerRow | null> {
  try {
    const item = await runtimeRequest<RuntimeMcpRecord>(`/v1/config/mcp/${id}`, {
      method: 'PUT',
      body: {
...
export async function softDelete(id: string, _deletedBy?: string): Promise<boolean> {
  try {

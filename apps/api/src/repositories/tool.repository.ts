 ...
 */
export async function findByIdAndOrg(id: string): Promise<ToolRow | null> {
  return findById(id)
}
 ...
 */
export async function findByNameAndOrg(name: string): Promise<ToolRow | null> {
  try {
...
export async function updateByOrg(
  ...
  const tool = await findByIdAndOrg(id)
  if (!tool) return null

  if (tool.is_builtin) {
    toolLogger.warn('[Security] 尝试修改内置 Tool', { id })
    return null
  }

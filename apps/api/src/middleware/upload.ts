...
export function handleFileUpload(req: UploadRequest, _res: Response, next: NextFunction): void {
  ...
  function tryFinalize() {
    ...
    if (fileLimitReached) {
      safeNext(createError(SKILL_UPLOAD_TOO_LARGE, `文件大小超过限制 (最大 ${Math.round(SKILL_MAX_SIZE_BYTES / 1024 / 1024)}MB)`))
      return
    }

    if (!fileReceived) {
      safeNext(createError(SKILL_UPLOAD_NO_FILE, '未检测到上传文件'))
      return
    }
  ...
  busboy.on('file', (fieldName: string, fileStream: NodeJS.ReadableStream, info: { filename: string; encoding: string; mimeType: string }) => {
    ...
    if (!isAllowedExtension(filename)) {
      fileStream.resume()
      safeNext(createError(SKILL_UPLOAD_INVALID_TYPE, `不支持的文件类型: ${filename}，仅支持 ${SKILL_UPLOAD_ALLOWED_EXTENSIONS.join(', ')}`))
      return
    }
    ...
    writeStream.on('error', (err) => {
      ...
      cleanupTempFile(tempPath)
      safeNext(createError('INTERNAL_ERROR', '文件写入失败'))
    })
  ...
  busboy.on('error', (err: Error) => {
    ...
    safeNext(createError('INTERNAL_ERROR', '文件上传解析失败'))
  })

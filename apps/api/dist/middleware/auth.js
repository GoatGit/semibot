...
import { AUTH_TOKEN_MISSING, AUTH_TOKEN_INVALID, AUTH_TOKEN_EXPIRED, AUTH_API_KEY_INVALID, AUTH_PERMISSION_DENIED, ERROR_HTTP_STATUS, ERROR_MESSAGES, } from '../constants/errorCodes.js';
...
export function requirePermission(...requiredPermissions) {
    return (req, res, next) => {
        ...
        if (!hasPermission) {
            console.warn(`[Auth] 权限不足 - 用户: ${user.userId}, 需要: ${requiredPermissions.join('|')}, 拥有: ${user.permissions.join(',')}`);
...
export function requireRole(...allowedRoles) {
    return (req, res, next) => {
        ...
        if (!allowedRoles.includes(user.role)) {
            console.warn(`[Auth] 角色不足 - 用户: ${user.userId}, 需要: ${allowedRoles.join('|')}, 拥有: ${user.role}`);
            sendAuthError(res, AUTH_PERMISSION_DENIED);
            return;
        }
        next();

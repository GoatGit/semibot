...
import { RATE_LIMIT_PER_MINUTE_USER, RATE_LIMIT_PER_MINUTE_ORG, RATE_LIMIT_WINDOW_MS, } from '../constants/config.js';
import { RATE_LIMIT_EXCEEDED, RATE_LIMIT_USER, RATE_LIMIT_ORG, ERROR_MESSAGES, } from '../constants/errorCodes.js';
// ═══════════════════════════════════════════════════════════════
 ...
 * 检查并更新限流状态
 */
function checkAndUpdateLimit(store, key, limit, windowMs) {
    const now = new Date();
    ...
    if (info.current >= limit) {
        console.warn(`[RateLimit] 限流触发 - Key: ${key}, 当前: ${info.current}, 限制: ${limit}`);
        return {
            allowed: false,
...
export function userRateLimit(req, res, next) {
    ...
    if (!userId) {
        ...
        const { allowed, info } = checkAndUpdateLimit(userLimits, `ip:${ip}`, RATE_LIMIT_PER_MINUTE_USER, RATE_LIMIT_WINDOW_MS);
    ...
    const { allowed, info } = checkAndUpdateLimit(userLimits, `user:${userId}`, RATE_LIMIT_PER_MINUTE_USER, RATE_LIMIT_WINDOW_MS);
    setRateLimitHeaders(res, info);
    if (!allowed) {
        sendRateLimitError(res, RATE_LIMIT_USER, info);
        return;
 ...
 * 组织级限流中间件
 */
export function orgRateLimit(req, res, next) {
    const orgId = req.user?.orgId;
    if (!orgId) {
        next();
        return;
    }
    const { allowed, info } = checkAndUpdateLimit(orgLimits, `org:${orgId}`, RATE_LIMIT_PER_MINUTE_ORG, RATE_LIMIT_WINDOW_MS);
    setRateLimitHeaders(res, info);
    if (!allowed) {
 ...
 */
export function combinedRateLimit(req, res, next) {
 ...
 * 发送限流错误响应
 */
function sendRateLimitError(res, code, info) {

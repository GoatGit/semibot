 ...
 */
router.post('/sessions/:sessionId', authenticate, combinedRateLimit, requirePermission('chat:write'), validate(chatMessageSchema, 'body'), asyncHandler(async (req, res) => {
 ...
 * 返回 SSE 流，事件同上
 */
router.post('/start', authenticate, combinedRateLimit, requirePermission('chat:write', 'sessions:write'), validate(startChatSchema, 'body'), asyncHandler(async (req, res) => {
    const orgId = req.user.orgId;
    const userId = req.user.userId;
    const { agentId, message } = req.body;
    // 创建新会话并处理聊天
    await chatService.startNewChat(orgId, userId, agentId, { message }, res);
}));

...
import { VALIDATION_MESSAGE_TOO_LONG, SSE_STREAM_ERROR, } from '../constants/errorCodes.js';
import { SSE_HEARTBEAT_INTERVAL_MS, MAX_MESSAGE_LENGTH, } from '../constants/config.js';
// ═══════════════════════════════════════════════════════════════
// SSE 连接管理
// ═══════════════════════════════════════════════════════════════
 ...
 * 创建 SSE 连接
 */
export function createSSEConnection(res, sessionId, userId) {
    const connection = {
        id: uuidv4(),
        res,
        sessionId,
 ...
 * 关闭 SSE 连接
 */
export function closeSSEConnection(connectionId) {
 ...
 * 发送 SSE 事件
 */
export function sendSSEEvent(connection, event, data) {
    if (!connection.isActive) {
        return false;
    }
    try {
 ...
 * 发送 Agent2UI 消息
 */
export function sendAgent2UIMessage(connection, type, data, metadata) {
    const message = {
        id: uuidv4(),
        type,
        data,
 ...
 * 处理聊天消息 (SSE 流式响应)
 */
export async function handleChat(orgId, userId, sessionId, input, res) {
    // 验证消息长度
    if (input.message.length > MAX_MESSAGE_LENGTH) {
        console.warn(`[Chat] 消息长度超出限制 - 长度: ${input.message.length}, 限制: ${MAX_MESSAGE_LENGTH}`);
 ...
 * 创建新会话并开始对话
 */
export async function startNewChat(orgId, userId, agentId, input, res) {
    // 验证 Agent

...
import { Check, Copy, FileCode, WrapText, ArrowLeftRight } from 'lucide-react'
    ...
    >
      ...
      <div
        className={clsx(
          ...
          'px-4 py-2',
          'border-b border-[rgba(255,255,255,0.08)] bg-[rgba(255,255,255,0.02)]'
        )}
      >
        ...
        <div className="flex items-center gap-1">
          <button
            ...
            aria-label={wrapLines ? t('agent2ui.code.horizontalScroll') : t('agent2ui.code.wrapCode')}
          >
            {wrapLines ? (
              <>
                <ArrowLeftRight className="w-3.5 h-3.5" />
                <span>{t('agent2ui.code.horizontalScroll')}</span>
              </>
            ) : (
              <>
                <WrapText className="w-3.5 h-3.5" />
                <span>{t('agent2ui.code.wrapCode')}</span>
              </>
            )}
          </button>
          <button
      ...
      {/* 代码区域 */}
      <div className={clsx(wrapLines ? 'overflow-x-hidden' : 'overflow-x-auto', 'bg-[rgba(0,0,0,0.22)]')}>
        <SyntaxHighlighter

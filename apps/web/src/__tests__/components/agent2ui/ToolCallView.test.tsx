...
vi.mock('@/components/providers/LocaleProvider', () => ({
  useLocale: () => ({
    ...
    t: (key: string, params?: Record<string, string | number>) => {
      switch (key) {
        case 'agent2ui.toolCall.running':
          return '执行中'
        case 'agent2ui.toolCall.success':
          return '成功'
        case 'agent2ui.toolCall.error':
          return '失败'
        case 'agent2ui.toolCall.successWithDuration':
          return `成功 · ${params?.seconds}s`
        case 'agent2ui.toolCall.errorWithDuration':
          return `失败 · ${params?.seconds}s`
        case 'agent2ui.toolCall.arguments':
          return '参数'
        case 'agent2ui.toolCall.result':
          return '结果'
        case 'common.retry':
          return '重试'
...
describe('ToolCallView', () => {
  ...
  describe('状态显示', () => {
    ...
    it('approval_pending 应该显示待审批状态而不是失败', () => {
      const data: ToolCallData = {
        ...baseData,
        status: 'error',
        result: {

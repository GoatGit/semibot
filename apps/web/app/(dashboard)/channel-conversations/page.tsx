...
type GatewaySource = 'telegram' | 'feishu' | 'channel'
      ...
      sortedConversations.forEach((conversation, index) => {
        ...
        if (runsPayload.length === 0) {
          group.runs.push({
            ...
            title: t('dashboard.channelList.runFallbackTitle'),
            status: conversation.status || 'unknown',
            updatedAt: conversation.updatedAt,
            href: `/channel-conversations/${encodeURIComponent(conversation.conversationId)}?provider=${source}&chatId=${encodeURIComponent(chatId || '')}`,
          })
        ...
        runsPayload.forEach((run) => {
          group.runs.push({
            ...
            runId: run.runId,
            conversationId: conversation.conversationId,
            title: run.resultSummary?.trim() || t('dashboard.channelList.runFallbackTitle'),
            status: run.status || 'unknown',
            updatedAt: run.updatedAt,
            href: `/channel-conversations/${encodeURIComponent(conversation.conversationId)}?provider=${source}&chatId=${encodeURIComponent(chatId || '')}&runId=${encodeURIComponent(run.runId)}`,
          })
          ...
          <CardContent className="p-6">
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2">
                <h1 className="text-2xl font-semibold text-text-primary">{t('dashboard.channelList.title')}</h1>
                <p className="text-sm text-text-secondary">{t('dashboard.channelList.subtitle')}</p>
              </div>
        ...
        <Card className="border-border-default">
          <CardContent className="p-4 space-y-3">
            <Input
              ...
              placeholder={t('dashboard.channelList.searchPlaceholder')}
              leftIcon={<Search size={16} />}
            />
            <div className="flex flex-wrap items-center gap-2">
              {(['all', 'telegram', 'feishu', 'channel'] as const).map((value) => (
                <button
                  key={value}
                  type="button"
                ...
                >
                  {value === 'all'
                    ? t('dashboard.channelList.filterAll')
        ...
        <div className="space-y-4">
          {isLoading ? (
            ...
            filteredGroups.map((group) => {
              ...
              const chatLabel = group.chatId || t('dashboard.channelList.unknownChat')
                  ...
                  <CardContent className="p-5 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                      ...
                      {group.runs[0] && (
                        ...
                        >
                          {t('dashboard.channelList.openLatest')}
                          <ArrowRight size={13} />
                        </Link>
                      )}
                    </div>
                    ...
                    {group.totalRuns > visibleRuns.length && (
                      <p className="text-xs text-text-tertiary">
                        {t('dashboard.channelList.showingRuns', { shown: visibleRuns.length, total: group.totalRuns })}
                      </p>
                    )}
                  </CardContent>
                </Card>

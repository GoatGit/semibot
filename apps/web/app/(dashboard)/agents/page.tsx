...
export default function AgentsPage() {
  ...
  // 获取 LLM 模型列表
  const { models: llmModels, loading: modelsLoading, error: modelsError, refetch: refetchModels } = useLLMModels()
  ...
  const openCreateForm = async () => {
    let nextModels = llmModels
    if (!modelsLoading && llmModels.length === 0) {
      await refetchModels()
    }
    nextModels = llmModels
    const defaultModel = nextModels.length > 0 ? nextModels[0].modelId : ''
  ...
  useEffect(() => {
    if (!showForm || formValues.model || llmModels.length === 0) return
    setFormValues((prev) => ({ ...prev, model: llmModels[0]?.modelId || '' }))
  }, [formValues.model, llmModels, showForm])
    ...
    <div className="flex-1 overflow-y-auto bg-bg-base">
      ...
      {showForm && (
        <AgentFormModal
          ...
          onReloadModels={() => void refetchModels()}
        ...
        <div>
          ...
          {!modelsLoading && models.length === 0 && (
            <div className="mt-2 flex items-center justify-between gap-3 text-xs text-text-tertiary">
              <span>{modelsError?.message || t('agents.modal.noModels')}</span>
              <button
                type="button"

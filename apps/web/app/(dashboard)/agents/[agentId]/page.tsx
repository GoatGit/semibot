...
import type { ApiResponse, Agent, NodeModelConfig, ModelRoleConfig } from '@/types'
...
type AgentDetailRecord = Agent & {
  config?: Agent['config'] & {
    modelRoles?: ModelRoleConfig
    studioSchema?: { inputs: { label: string; description?: string }[] }
  }
  mcpServerIds?: string[]
  runtimeType?: 'semigraph'
}
...
const EMPTY_MODEL_ROLES: ModelRoleConfig = {
  plan: {},
  act: {},
  textProcessing: {},
}
...
export default function AgentDetailPage() {
  ...
  const { models, loading: modelsLoading, error: modelsError, refetch: refetchModels } = useLLMModels()
  ...
  useEffect(() => {
    ...
    const loadAgent = async () => {
      try {
        setIsLoading(true)
        setError(null)
        const response = await apiClient.get<ApiResponse<AgentDetailRecord>>(`/agents/${agentId}`)
        ...
        setModelRoles({
          plan: agent.config?.modelRoles?.plan ?? {},
        ...
        setStudioInputs(
          (agent.config?.studioSchema?.inputs ?? []).map((h) => ({
            label: h.label,
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 校验 selectedSkills：过滤掉不在 availableSkillDefs 中的 skillId
      const validSkillIds = new Set(availableSkillDefs.map((def) => def.id))
      const validatedSkills = selectedSkills.filter((id) => validSkillIds.has(id))
      const removedSkills = selectedSkills.filter((id) => !validSkillIds.has(id))
      if (removedSkills.length > 0) {
        toast.warning(
          '部分技能已失效',
          `已自动移除 ${removedSkills.length} 个不可用的技能`
        )
      }
      ...
      if (isNew) {
        ...
        const response = await apiClient.put<ApiResponse<Agent>>(`/agents/${agentId}`, {
          name: payload.name,
          description: payload.description,
          systemPrompt: payload.systemPrompt,
          runtimeType: payload.runtimeType,
    ...
    <div className="flex flex-col flex-1 min-h-0 bg-bg-base">
      <div className="mx-auto flex w-full max-w-6xl flex-1 min-h-0 flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-6">
                ...
                <div>
                  ...
                  {!modelsLoading && models.length === 0 && (
                    <div className="mt-2 flex items-center justify-between gap-3 text-xs text-text-tertiary">
                      <span>{modelsError?.message || t('agentsDetail.noModels')}</span>
                      <button
                        type="button"
          ...
          <Card className="max-w-3xl mt-6">
          <CardHeader>
            <div className="text-lg font-semibold text-text-primary">{t('agentsDetail.modelRolesTitle')}</div>
            <CardDescription>{t('agentsDetail.modelRolesDescription')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-5">
              <RoleModelRow
                role="plan"
                label={t('agentsDetail.modelRolesPlan')}
                config={modelRoles.plan ?? {}}
              ...
              <RoleModelRow
                role="act"
                label={t('agentsDetail.modelRolesAct')}
                config={modelRoles.act ?? {}}
                modelOptions={modelOptions}
                modelsLoading={modelsLoading}
                isSaving={isSaving}
                onChange={handleRoleChange}
              ...
              <RoleModelRow
                role="textProcessing"
                label={t('agentsDetail.modelRolesTextProcessing')}
                config={modelRoles.textProcessing ?? {}}
                modelOptions={modelOptions}
                modelsLoading={modelsLoading}
                isSaving={isSaving}
                onChange={handleRoleChange}
          ...
          <Card className="max-w-3xl mt-6">
          <CardHeader>
            <div className="text-lg font-semibold text-text-primary">{t('agentsDetail.studioSchemaTitle')}</div>
            <CardDescription>{t('agentsDetail.studioSchemaDescription')}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-2">
              {studioInputs.map((hint, i) => (
                <div key={i} className="flex items-start gap-2">
                  <div className="flex-1 grid grid-cols-2 gap-2">
                    <Input
                      value={hint.label}
                      onChange={(e) => setStudioInputs((prev) => prev.map((h, idx) => idx === i ? { ...h, label: e.target.value } : h))}
                      placeholder={t('agentsDetail.studioInputLabel')}
                      disabled={isSaving}
                    />
                    <Input
                      value={hint.description}
                      onChange={(e) => setStudioInputs((prev) => prev.map((h, idx) => idx === i ? { ...h, description: e.target.value } : h))}
                      placeholder={t('agentsDetail.studioInputDescription')}
                      disabled={isSaving}
                    />
                  </div>
                  <button
                    type="button"
                    onClick={() => setStudioInputs((prev) => prev.filter((_, idx) => idx !== i))}
                    className="p-2 text-text-tertiary hover:text-error-500 transition-colors mt-0.5"
                    disabled={isSaving}
                  >
                    <Trash2 size={14} />
                  </button>
...
function RoleModelRow({
  ...
  modelOptions: SelectGroup[]
  modelsLoading: boolean
  isSaving: boolean
  onChange: (role: keyof ModelRoleConfig, patch: NodeModelConfig) => void
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div>
          <label className="block text-xs text-text-tertiary mb-1">{t('agentsDetail.modelRolesModelLabel')}</label>
          <Select
            value={config.model ?? ''}
            onChange={(val) => onChange(role, { ...config, model: val || undefined })}
            disabled={isSaving || modelsLoading}
            options={modelOptions}
            placeholder={t('agentsDetail.modelRolesModelPlaceholder')}
          />
        </div>
        <div>
          <label className="block text-xs text-text-tertiary mb-1">{t('agentsDetail.modelRolesTemperatureLabel')}</label>
          <input
            type="number"
            min={0}
            max={2}
            ...
            disabled={isSaving}
            placeholder={t('agentsDetail.modelRolesTemperaturePlaceholder')}
            className={clsx(

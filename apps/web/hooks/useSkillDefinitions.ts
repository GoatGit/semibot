...
interface SkillDefinitionsCacheValue {
  rows: SkillDefinition[]
  meta?: ApiResponse<SkillDefinition[]>['meta']
}

// 技能列表变化不频繁，缓存 3 分钟
const SKILLS_CACHE_TTL_MS = 3 * 60_000
const SKILLS_CACHE_KEY = 'skills:definitions'
...
export function useSkillDefinitions(options: UseSkillDefinitionsOptions = {}) {
  ...
  const stableOptions = JSON.parse(optionsKey) as UseSkillDefinitionsOptions
  ...
  const fetchDefinitions = useCallback(async (forceRefresh = false) => {
    try {
      ...
      const cacheKey = `${SKILLS_CACHE_KEY}:${JSON.stringify(stableOptions)}`
      if (forceRefresh) invalidateCache(cacheKey)

      const cached = await cachedFetch<SkillDefinitionsCacheValue>(
        cacheKey,
        async () => {
      ...
      setDefinitions(cached.rows)
      setMeta(
        cached.meta || {
          total: cached.rows.length,
          page: 1,
          limit: cached.rows.length,
          totalPages: 1,
        }
      )
    } catch (err) {

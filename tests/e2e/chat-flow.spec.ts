...
test.describe('Chat Flow', () => {
  ...
  test.describe('HITL Approval Flow', () => {
    test('should approve browser automation action and resume execution', async ({ page }) => {
      ...
      await expect(page.getByRole('button', { name: '通过', exact: true })).toBeEnabled({ timeout: 10000 })
    ...
    test('should not show approval_denied before approving browser automation for today news search', async ({ page }) => {
      await mockSessionList(page, [{ id: 'sess-news-hitl', title: 'News HITL', createdAt: new Date().toISOString() }])
      await mockSessionDetail(page, 'sess-news-hitl', [])
      ...
      await page.route('**/api/v1/approvals**', async (route, request) => {
        ...
        const items = approvalState === 'pending'
          ? [{
              id: 'ap-news-browser-1',
              status: 'pending',
              event_id: 'sess-news-hitl:browser_automation',
              risk_level: 'high',
              created_at: new Date().toISOString(),
            }]
          : []
      ...
      await page.route('**/api/v1/chat/sessions/sess-news-hitl', async (route, request) => {
        const payload = request.postDataJSON() as { message?: string } | null
        const message = (payload?.message || '').trim()
        ...
        await route.fulfill({
          ...
          body: buildTextSSE('sess-news-hitl', [
            'browser_automation 调用被策略拦截，等待人工审批。',
            '浏览器自动化请求已暂停，等待审批。',
          ]),
        })
      ...
      await page.route('**/api/v1/approvals/ap-news-browser-1/approve', async (route, request) => {
        if (request.method() !== 'POST') {
          await route.continue()
          return
        }
      ...
      await page.goto('/chat/sess-news-hitl')
      await page.getByPlaceholder('输入您的问题...').fill('搜索今天新闻')
      await page.getByRole('button', { name: '发送' }).click()

      await expect(page.getByText('浏览器自动化请求已暂停，等待审批。')).toBeVisible({ timeout: 10000 })
      await expect(page.getByText('待审批操作 1')).toBeVisible({ timeout: 10000 })
      await expect(page.getByText('ap-news-browser-1').first()).toBeVisible()
